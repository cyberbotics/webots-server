...
export default class ProtoManager {
  ...
  async loadX3d() {
    ...
    if (
      this.proto.isRoot &&
      ['PBRAppearance', 'Appearance'].includes(this.proto.getBaseNode().name)
    ) {
      ...
      [
        ...
        'PointSet',
        'Sphere',
      ].includes(this.proto.getBaseNode().name)
    ) {
      const wrapper = this.#createGeometryWrapper();
      scene.appendChild(wrapper);
    } else scene.appendChild(this.proto.toX3d());
  ...
  exportProto(name, fieldsToExport) {
    ...
    function listExternProto(node, list) {
      for (const parameter of node.parameters.values()) {
        ...
        if (currentValue instanceof SFNode && currentValue.value !== null) {
          if (
            currentValue.value.isProto &&
            !list.includes(currentValue.value.url)
          ) {
    ...
    for (const item of externProto) s += `EXTERNPROTO "${item}"\n`;
    ...
    for (const parameter of this.proto.parameters.values()) {
      if (fieldsToExport.has(parameter.name))
        s += `${indent(2)}field ${stringifyType(parameter.type)} ${
          parameter.name
        } ${parameter.value.toVrml()}\n`;
    }
  ...
  #createGeometryWrapper() {
    ...
    imageTexture.setAttribute(
      'url',
      'https://raw.githubusercontent.com/cyberbotics/webots/released/projects/default/worlds/textures/tagged_wall.jpg'
    );
    imageTexture.setAttribute('role', 'baseColorMap');
    appearance.appendChild(imageTexture);
