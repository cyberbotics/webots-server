...
export default class NodeSelectorWindow {
  ...
  #setupWindow(parentNode) {
    ...
    img.draggable = false;
    img.src =
      'https://raw.githubusercontent.com/cyberbotics/webots/R2023b/resources/images/missing_proto_icon.png';
    ...
    acceptButton.onclick = () => {
      if (typeof this.selection === 'undefined') return;

      this.insertNode(
        this.selection.innerText,
        this.selection.baseNode ? this.baseNodes : this.protoNodes
      );
    };
  ...
  async fetchCompatibleNodes() {
    ...
    if (!url.includes('webots.cloud')) url = 'https://webots.cloud/';
    ...
    if (!this.isRobotDescendant()) content.skip_no_robot_ancestor = true;

    return fetch('https://' + url + '/ajax/proto/insertable.php', {
      method: 'post',
      ...
      .then(json => {
        ...
        for (const baseType of content.base_types) {
          if (Object.keys(FieldModel).includes(baseType)) {
            ...
            this.baseNodes.set(
              baseType,
              new NodeInfo(baseType, baseType, license, description, icon)
            );
          }
        }
      });
  ...
  populateWindow() {
    ...
    for (const [name, info] of this.baseNodes) {
      ...
      if (!this.doFieldRestrictionsAllowNode(name)) continue;

      // don't display base nodes nodes which have been filtered-out by the user's "filter" widget
      if (
        !info.url.toLowerCase().includes(filterInput.value) &&
        !info.baseType.toLowerCase().includes(filterInput.value)
      )
        continue;
    ...
    if (
      baseNodeItems.children.length === 0 &&
      protoItems.children.length === 0
    ) {
      ...
      if (this.parameter.type === VRML.SFNode) {
        if (this.parameter.value.value === null) {
          ...
          else this.selection = protoItems.children[0];
        } else {
          let index = compatibleBaseNodes.indexOf(
            this.parameter.value.value.name
          );
          if (index === -1) {
            index = compatibleProtoNodes.indexOf(
              this.parameter.value.value.name
            );
            if (index !== -1) this.selection = protoItems.children[index];
          } else this.selection = baseNodeItems.children[index];
        }
  ...
  populateNodeInfo(nodeName) {
    ...
    if (typeof nodeName === 'undefined') {
      ...
      const info = this.baseNodes.has(nodeName)
        ? this.baseNodes.get(nodeName)
        : this.protoNodes.get(nodeName);
  ...
  doFieldRestrictionsAllowNode(nodeName) {
    if (this.parameter.restrictions.length === 0) return true;
    ...
    for (const restriction of this.parameter.restrictions) {
      if (this.parameter.type === VRML.MFNode) {
        for (const child of restriction.value) {
          if (nodeName === child.value.name) return true;
        }
      } else if (nodeName === restriction.value.name) return true;
    }
  ...
  getAllowedBaseType() {
    if (typeof this.parameter === 'undefined')
      throw new Error(
        'The parameter is expected to be defined prior to checking node compatibility.'
      );
    ...
    if (fieldName === 'appearance') baseType = ['Appearance', 'PBRAppearance'];
    ...
    else if (fieldName === 'textureTransform') baseType = ['TextureTransform'];
    ...
    } else if (fieldName === 'physics') baseType = ['Physics'];
    else if (fieldName === 'immersionProperties')
      baseType = ['ImmersionProperties'];
    else if (fieldName === 'damping') baseType = ['Damping'];
    else if (fieldName === 'lens') baseType = ['Lens'];
    else if (fieldName === 'focus') baseType = ['Focus'];
    else if (fieldName === 'zoom') baseType = ['Zoom'];
    else if (fieldName === 'recognition') baseType = ['Recognition'];
    else if (fieldName === 'lensFlare') baseType = ['LensFlare'];
    else if (fieldName === 'material') baseType = ['Material'];
  ...
  isTransformOrTransformDescendant(field) {
    ...
    if (node.isProto) node = node.baseType;

    if (node.name === 'Transform') return true;
  ...
  getSlotType() {
    ...
    } else parentNode = this.parameter.node;

    let slotType = parentNode.fields
      .get('type')
  ...
  show(parameter, element, callback, parent, mfId, resetButton) {
    ...
    if (window.webotsJSPreventAddNode) {
      alert(
        'New nodes can only be inserted in protos that are registered on webots.cloud.'
      );
      return;
    }
    ...
    this.fetchCompatibleNodes().then(() => this.populateWindow());
  ...
  isInBoundingObject(fieldName, parentNode) {
    if (fieldName === 'boundingObject') return true;
    ...
    while (typeof node !== 'undefined' && typeof parentField !== 'undefined') {
      while (
        parentField instanceof Parameter &&
        parentField.aliasLinks.length > 0
      )
        parentField = parentField.aliasLinks[0];

      if (parentField.name === 'boundingObject') return true;
