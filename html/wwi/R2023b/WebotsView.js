...
export default class WebotsView extends HTMLElement {
  ...
  connectedCallback() {
    if (this.#initialCallbackDone) return;
    ...
    this.progressCss.href =
      'https://cyberbotics.com/wwi/R2023b/css/progress.css';
  ...
  #init() {
      ...
      Promise.all(promises).then(() => {
        ...
        if (
          ...
          this.loadAnimation(
            scene,
            animation,
            !(this.dataset.autoplay && this.dataset.autoplay === 'false'),
            isMobileDevice,
            thumbnail
          );
    ...
    promises.push(
      this.#loadScript(
        'https://cyberbotics.com/wwi/R2023b/dependencies/ansi_up.js'
      )
    );
    promises.push(
      this.#loadScript(
        'https://cyberbotics.com/wwi/R2023b/dependencies/assimpjs.js'
      )
    );
    promises.push(
      this.#loadScript(
        'https://cyberbotics.com/wwi/R2023b/dependencies/glm-js.min.js'
      )
    );
    promises.push(
      this.#loadScript(
        'https://cyberbotics.com/wwi/R2023b/dependencies/quaternion.min.js'
      )
    );
    promises.push(
      this.#loadScript(
        'https://cyberbotics.com/wwi/R2023b/dependencies/libtess.min.js'
      )
    );
    promises.push(
      this.#loadScript('https://cyberbotics.com/wwi/R2023b/enum.js')
    );
    promises.push(
      this.#loadScript('https://cyberbotics.com/wwi/R2023b/wrenjs.js')
    );
  }
  ...
  close() {
    if (this.#hasAnimation) this.#closeAnimation();
    else if (this.#hasScene || this.#hasProto) this.#closeScene();
    else if (
      typeof this._view !== 'undefined' &&
      typeof this._view.stream !== 'undefined' &&
  ...
  resetViewpoint() {
    if (
      ...
      typeof WbWorld.instance.viewpoint !== 'undefined'
    ) {
  ...
  updateNode(nodeId, field, value, render) {
    ...
    if (render) this._view.x3dScene.render();
  }

  getNode(id) {
    if (
      typeof WbWorld.instance !== 'undefined' &&
      WbWorld.instance.nodes !== 'undefined'
    )
  ...
  setAmbientOcclusion(level) {
    ...
    if (typeof this.toolbar !== 'undefined') {
      this.toolbar.changeGtao({
        srcElement: { id: this.toolbar.gtaoLevelToText(level) },
      });
      this.toolbar.settingsPane.style.visibility = 'hidden';
    } else changeGtaoLevel(level);
  }
  ...
  loadAnimation(scene, animation, play, isMobileDevice, thumbnail, raw) {
    ...
    if (!this.initializationComplete)
      ...
      else this._view.setAnimation(animation, 'play', true, raw);
  ...
  setAnimationStepCallback(callbackFunction) {
    if (
      typeof this._view !== 'undefined' &&
      typeof this._view.animation !== 'undefined'
    ) {
  ...
  connect(server, mode, broadcast, isMobileDevice, timeout, thumbnail) {
    ...
    if (!this.initializationComplete)
      ...
      if (typeof timeout === 'undefined') timeout = -1; // disable timeout that stops the simulation after a given time
      ...
      this._view.onready = () => {
        ...
        if (document.getElementById('robot-window-button') !== null)
          document
            .getElementsByTagName('webots-view')[0]
            .toolbar.loadRobotWindows();
        if (typeof this.onready === 'function') this.onready();
      };
  ...
  #disconnect() {
    ...
    this._view.close();
    this.innerHTML = null;
    if (this._view.mode === 'mjpeg') this._view.multimediaClient = undefined;
  ...
  sendMessage(message) {
    if (
      typeof this._view !== 'undefined' &&
      this._view.stream.socket.readyState === 1
    )
  ...
  loadProto(proto, isMobileDevice, thumbnail, moveFloor) {
    ...
    if (!this.initializationComplete)
      setTimeout(
        () => this.loadProto(proto, isMobileDevice, thumbnail, moveFloor),
        500
      );
    else {
      // terminate the previous activity if any
      ...
      this._view.onready = async () => {
        ...
        const topProtoNode =
          WbWorld.instance.root.children[
            WbWorld.instance.root.children.length - 1
          ];
        if (
          topProtoNode instanceof WbDevice ||
          topProtoNode.nodeType === WbNodeType.WB_NODE_SLOT ||
          topProtoNode.nodeType === WbNodeType.WB_NODE_SHAPE ||
          moveFloor === '1'
        )
          this.#repositionFloor(
            topProtoNode,
            WbWorld.instance.root.children[
              WbWorld.instance.root.children.length - 2
            ]
          );

        WbWorld.instance.viewpoint.moveViewpointToObject(topProtoNode);
        WbWorld.instance.viewpoint.defaultPosition =
          WbWorld.instance.viewpoint.position;
        this._view.x3dScene.render();
      };
  ...
  #repositionFloor(proto, floor) {
    ...
    if (typeof boundingSphere === 'undefined') return;

    boundingSphere.recomputeIfNeeded(false);
    if (boundingSphere.isEmpty()) return;
