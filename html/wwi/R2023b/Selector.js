...
export default class Selector {
  static select(id) {
    ...
    if (
      Selector.previousAncestor === Selector.topSolidId(node) &&
      (!Selector.local || Selector.preciseId !== 'n' + id)
    ) {
  ...
  static checkIfParentIsSelected(node) {
    ...
    if (typeof parent !== 'undefined') {
      if (Selector.selectedId === parent.id) return true;
  ...
  static firstSolidId(node) {
    if (typeof node !== 'undefined') {
      if (node instanceof WbSolid) return node.id;
      else if (
        typeof node.parent !== 'undefined' &&
        typeof WbWorld.instance.nodes.get(node.parent) !== 'undefined'
      )
  ...
  static topSolidId(node) {
    ...
    while (typeof currentNode !== 'undefined') {
      if (currentNode instanceof WbSolid) topSolid = currentNode.id;
