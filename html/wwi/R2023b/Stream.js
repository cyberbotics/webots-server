...
export default class Stream {
  ...
  connect() {
    ...
    this.view.progress.setProgressBar(
      'block',
      'Connecting to Webots instance...',
      percent,
      'Opening socket...',
    );
  ...
  #onSocketOpen(event) {
    ...
    if (mode === 'mjpeg') {
      mode += ': '
        + this.view.view3D.offsetWidth
        + 'x'
        + this.view.view3D.offsetHeight;
    } else if (this.view.broadcast) mode += ';broadcast';
  ...
  #onSocketClose(event) {
    this.view.onerror(
      'Disconnected from ' + this.wsServer + ' (' + event.code + ')',
    );
    // https://tools.ietf.org/html/rfc6455#section-7.4.1
    if (
      (event.code > 1001 && event.code < 1016)
      || (event.code === 1001 && this.view.quitting === false)
    ) {
      if (
        window.confirm(
          `Streaming server error
      Connection closed abnormally. (Error code:`
            + event.code
            + `)
  ...
  #onSocketMessage(event) {
    ...
    if (data.startsWith('robot window:')) {
      ...
      if (json.remove) {
        ...
        webots.currentView.robots.push({
          name: json.robot,
          window: robotWindow,
          main: json.main,
    ...
    } else if (data.startsWith('robot:')) return 0;
    // We need to keep this condition, otherwise the robot window messages will be printed as errors.
      ...
      if (document.getElementById('webots-clock')) {
        document.getElementById(
          'webots-clock',
        ).innerHTML = webots.parseMillisecondsIntoReadableTime(0);
      }
      ...
      if (
        typeof this.view.x3dScene !== 'undefined'
        && typeof this.view.multimediaClient === 'undefined'
      ) {
      ...
      if (document.getElementById('webots-clock')) {
        document.getElementById(
          'webots-clock',
        ).innerHTML = webots.parseMillisecondsIntoReadableTime(this.view.time);
      }
    } else if (data === 'delete world') this.view.destroyWorld();
    else {
      let messagedProcessed = false;
      if (typeof this.view.multimediaClient !== 'undefined') {
        messagedProcessed = this.view.multimediaClient.processServerMessage(
          data,
        );
      } else if (typeof this.view.x3dScene !== 'undefined') {
        messagedProcessed = this.view.x3dScene.processServerMessage(
          data,
          this.view,
        );
      }
      if (!messagedProcessed) {
        console.log(
          'WebSocket error: Unknown message received: "' + data + '"',
        );
      }
    }
  }
