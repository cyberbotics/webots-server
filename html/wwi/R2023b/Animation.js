...
export default class Animation {
  ...
  constructor(jsonPromise, scene, view, gui, loop) {
    ...
    this.#view = view;
    this.gui =
      typeof gui === 'undefined' || gui === 'play' ? 'real-time' : 'pause';
  ...
  init(onReady) {
    ...
    if (this.#jsonPromise instanceof Promise) {
      this.#jsonPromise
        .then(json => this.#setup(json))
        .catch(error => console.error(error));
    } else this.#setup(this.#jsonPromise);
  }
  ...
  #setup(data) {
    this.data = data;
    this.#labelsIds =
      typeof this.data.labelsIds === 'undefined'
        ? []
        : this.data.labelsIds
            .split(';')
            .filter(Boolean)
    ...
    if (this.data.frames.length > 1000) {
      ...
      for (let i = 0; i < this.numberOfKeyFrames; i++) {
        ...
        for (
          let j = (i + 1) * this.keyFrameStepSize;
          j > i * this.keyFrameStepSize;
          j--
        ) {
          const updates = this.data.frames[j].updates;
          if (updates) {
            for (let k = 0; k < updates.length; k++) {
              const currentIdFields = allUpdates.has(updates[k].id)
                ? allUpdates.get(updates[k].id)
                : new Map();
              for (let field in updates[k]) {
                if (field === 'id' || currentIdFields.has(field)) continue;
                else
                  currentIdFields.set(field, {
                    id: updates[k].id,
                    [field]: updates[k][field],
                  });
              }
          ...
          if (allLabels.size === this.#labelsIds.length)
            // We already have all the update we need for the labels
            continue;
        ...
        if (i === 0) {
          ...
          if (updates) {
            for (let j = 0; j < updates.length; j++) {
              const currentIdFields = allUpdates.has(updates[j].id)
                ? allUpdates.get(updates[j].id)
                : new Map();
          ...
          // Check the previous keyFrame to get missing updates
          const updates = this.#keyFrames.get(i - 1).updates;
          for (let element of updates) {
            const id = element[0];
            const currentIdFields = allUpdates.has(id)
              ? allUpdates.get(id)
              : new Map();

            for (let field of element[1]) {
              if (currentIdFields.has(field[0]))
                // we want to update each field only once
                continue;
              else currentIdFields.set(field[0], field[1]);
            }
          ...
          for (let label of labels) {
            const id = label[0];
            if (!allLabels.has(id)) allLabels.set(id, label[1]);
          }
        }

        this.#keyFrames.set(i, {
          updates: new Map(allUpdates),
          labels: new Map(allLabels),
        });
      }
      this.numberOfFields = new Map();
      for (let [key, value] of this.#keyFrames.get(this.numberOfKeyFrames - 1)
        .updates)
  ...
  updateAnimationState(requestedStep = undefined) {
    ...
    if (automaticMove) {
      requestedStep = Math.floor(
        (this.#elapsedTime() * this.speed) / this.data.basicTimeStep
      );
    ...
    if (requestedStep !== this.step) {
      ...
      if (this.step !== this.#previousStep + 1) {
        ...
        const closestKeyFrame =
          Math.floor(this.step / this.keyFrameStepSize) - 1;

        let previousStepIsAKeyFrame = false;
        if (
          this.step > this.#previousStep &&
          this.#previousStep > (closestKeyFrame + 1) * this.keyFrameStepSize
        )
          previousUpdateStep = this.#previousStep;
        else {
          previousUpdateStep = (closestKeyFrame + 1) * this.keyFrameStepSize;
          if (this.#keyFrames.size > 0) previousStepIsAKeyFrame = true;
        }
        ...
        for (let i = this.step; i >= previousUpdateStep; i--) {
          if (this.data.frames[i].updates) {
            for (let j = 0; j < this.data.frames[i].updates.length; j++) {
              // At each frame, apply all updates
              const id = this.data.frames[i].updates[j].id;
              if (!completeIds.has(id)) {
                // Try to apply some updates to a node only if it is missing some
                for (let field in this.data.frames[i].updates[j]) {
                  ...
                  if (appliedFieldsByIds.get(id).has(field))
                    // we want to update each field only once
                    continue;
                  else {
                    this.#scene.applyUpdate({
                      id: id,
                      [field]: this.data.frames[i].updates[j][field],
                    });
                    appliedFieldsByIds.get(id).add(field);

                    if (
                      typeof this.numberOfFields !== 'undefined' &&
                      appliedFieldsByIds.size === this.numberOfFields.get(id)
                    )
                      completeIds.add(id);
                  }
                }
        ...
        if (previousStepIsAKeyFrame && closestKeyFrame >= 0) {
          // Get the missing updates from the closest keyFrame
          ...
          for (let element of updates) {
            ...
            if (!completeIds.has(id)) {
              ...
              for (let field of element[1]) {
                ...
                if (appliedFieldsByIds.get(id).has(field[0]))
                  // we want to update each field only once
                  continue;
                else {
