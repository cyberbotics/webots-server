...
export default class WbTesselator {
  ...
  static tesselate(indexes, vertices, results) {
    ...
    if (indexSize !== vertexSize) return;

    const tesselator = new libtess.GluTesselator();
    if (typeof tesselator === 'undefined') return;

    tesselator.gluTessCallback(
      libtess.gluEnum.GLU_TESS_BEGIN,
      WbTesselator.tessBegin
    );
    tesselator.gluTessCallback(
      libtess.gluEnum.GLU_TESS_VERTEX_DATA,
      WbTesselator.tessVertexData
    );
    tesselator.gluTessCallback(
      libtess.gluEnum.GLU_TESS_END,
      WbTesselator.tessEnd
    );
    tesselator.gluTessCallback(
      libtess.gluEnum.GLU_TESS_EDGE_FLAG,
      WbTesselator.tessEdgeFlag
    );
    tesselator.gluTessCallback(
      libtess.gluEnum.GLU_TESS_ERROR,
      WbTesselator.tessError
    );
    ...
    if (results.length % 3 !== 0 || WbTesselator.error) {
      console.error(
        'Tessellation Error: the result of the tesselation is not a multiple of 3.'
      );
      results = [];
    }
  }
