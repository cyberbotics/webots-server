...
export default class WbWrenPicker {
  ...
  pick(x, y) {
    ...
    _wr_viewport_set_camera(
      this.#viewportDepth,
      _wr_viewport_get_camera(viewport)
    );
    ...
    _wr_frame_buffer_copy_depth_pixel(
      this.#frameBufferDepth,
      x,
      y,
      dataPointer,
      true
  ...
  static setPickable(renderable, uniqueId, pickable) {
    ...
    let material = Module.ccall(
      'wr_renderable_get_material',
      'number',
      ['number', 'string'],
      [renderable, 'picking']
    );
    ...
    if (!material) {
      ...
      Module.ccall(
        'wr_renderable_set_material',
        null,
        ['number', 'number', 'string'],
        [renderable, material, 'picking']
      );
    }

    let depthMaterial = Module.ccall(
      'wr_renderable_get_material',
      'number',
      ['number', 'string'],
      [renderable, 'depth']
    ...
    if (!depthMaterial) {
      ...
      _wr_material_set_default_program(
        depthMaterial,
      ...
      Module.ccall(
        ...
        null,
        ['number', 'number', 'string'],
        [renderable, depthMaterial, 'depth']
      );
    }
    ...
    const encodedIdPointerSecondHalf = arrayXPointerFloat([
      encodedId[3],
      encodedId[4],
      encodedId[5],
    ]);
  ...
  #setup() {
    ...
    _wr_frame_buffer_append_output_texture(
      this.#frameBuffer,
      this.#outputTexture
    );
    ...
    _wr_texture_set_internal_format(
      this.#outputTextureDepth,
      Enum.WR_TEXTURE_INTERNAL_FORMAT_R32F
    );

    _wr_frame_buffer_set_size(
      this.#frameBufferDepth,
      this.#width,
      this.#height
    );
