...
export default class X3dScene {
  ...
  render(toRemoveGtaoArtifact) {
    ...
    if (this.#nextRenderingTime && this.#nextRenderingTime > currentTime) {
      if (!this.#renderingTimeout)
        this.#renderingTimeout = setTimeout(
          () => this.render(toRemoveGtaoArtifact),
          this.#nextRenderingTime - currentTime
        );
      return;
    }
    ...
    if (toRemoveGtaoArtifact) --this.remainingRenderings;
    else this.remainingRenderings = 10;

    if (this.remainingRenderings > 0) setTimeout(() => this.render(true), 80);
  }
  ...
  resize() {
    ...
    if (typeof WbWorld.instance === 'undefined') return;
  ...
  destroyWorld() {
    if (
      typeof document.getElementsByTagName('webots-view')[0] !== 'undefined' &&
      typeof document.getElementsByTagName('webots-view')[0].toolbar !==
        'undefined'
    ) {
  ...
  #deleteObject(id) {
    ...
    if (typeof object !== 'undefined') object.delete();

    WbWorld.instance.robots.forEach((robot, i) => {
      if (robot.id === 'n' + id) WbWorld.instance.robots.splice(i, 1);
    });
  ...
  loadWorldFile(url, onLoad, progress) {
    ...
    xmlhttp.onerror = () => {
      progress.setProgressBar(
        ...
        'Loading world file...',
        5,
        'An unknown error occurred during the loading...'
      );
    };
    xmlhttp.send();
  }
  ...
  async loadObject(x3dObject, parentId, callback) {
    ...
    if (typeof node === 'undefined') return; // can happen for nodes that have no webotsjs counterpart, like Physics
  ...
  applyUpdate(update) {
    ...
    if (typeof id === 'string') id = id.replace('n', '');
    ...
    while (length >= 0) {
      ...
      } else this.#applyUpdateToObject(update, use);
  ...
  #applyUpdateToObject(update, object) {
    for (let key in update) {
      ...
      if (key === 'translation') {
        ...
        if (nodeType === WbNodeType.WB_NODE_TEXTURE_TRANSFORM)
          ...
          else object.rotation = quaternion;
        }
      } else if (key === 'scale') {
        if (object instanceof WbPose)
        ...
        if (
          object instanceof WbLight ||
          nodeType === WbNodeType.WB_NODE_CAD_SHAPE ||
          nodeType === WbNodeType.WB_NODE_SHAPE
        )
        ...
        if (
          nodeType === WbNodeType.WB_NODE_BOX ||
          nodeType === WbNodeType.WB_NODE_PLANE
        )
          object.size = convertStringToVec3(update[key]);
      } else if (key === 'radius') {
        if (
          nodeType === WbNodeType.WB_NODE_CAPSULE ||
          nodeType === WbNodeType.WB_NODE_SPHERE ||
          nodeType === WbNodeType.WB_NODE_CYLINDER ||
          nodeType === WbNodeType.WB_NODE_SPOT_LIGHT ||
        ...
        if (
          nodeType === WbNodeType.WB_NODE_SPHERE ||
          nodeType === WbNodeType.WB_NODE_CAPSULE ||
          nodeType === WbNodeType.WB_NODE_CONE ||
          nodeType === WbNodeType.WB_NODE_CYLINDER
        )
        ...
        if (
          nodeType === WbNodeType.WB_NODE_MESH ||
          nodeType === WbNodeType.WB_NODE_INDEXED_FACE_SET ||
          nodeType === WbNodeType.WB_NODE_CAD_SHAPE
        )
        ...
        if (nodeType === WbNodeType.WB_NODE_MESH) object.name = update[key];
        ...
        if (
          ...
          object.url = urlString.split('"').filter(element => {
        ...
        if (
          nodeType === WbNodeType.WB_NODE_INDEXED_LINE_SET ||
          nodeType === WbNodeType.WB_NODE_INDEXED_FACE_SET
        )
        ...
        else if (key === 'fogType') object.fogType = update[key];
      } else if (nodeType === WbNodeType.WB_NODE_SPOT_LIGHT) {
        if (key === 'beamWidth') object.beamWidth = parseFloat(update[key]);
        ...
        else if (key === 'normalIndex') object.normalIndex = update[key];
        else if (key === 'texCoordIndex') object.texCoordIndex = update[key];
        else if (key === 'creaseAngle')
          object.creaseAngle = parseFloat(update[key]);
      } else if (nodeType === WbNodeType.WB_NODE_ELEVATION_GRID) {
        if (key === 'xDimension') object.xDimension = update[key];
        else if (key === 'xSpacing') object.xSpacing = update[key];
        else if (key === 'yDimension') object.yDimension = update[key];
        else if (key === 'ySpacing') object.ySpacing = update[key];
        else if (key === 'thickness') object.thickness = update[key];
      } else if (
        nodeType === WbNodeType.WB_NODE_PBR_APPEARANCE ||
        nodeType === WbNodeType.WB_NODE_MATERIAL
      ) {
        if (key === 'baseColor')
        ...
        else if (key === 'filtering') object.filtering = parseInt(update[key]);
      } else if (nodeType === WbNodeType.WB_NODE_CAMERA) {
        if (key === 'far') object.far = parseFloat(update[key]);
        else if (key === 'near') object.near = parseFloat(update[key]);
      } else if (nodeType === WbNodeType.WB_NODE_RANGE_FINDER) {
        if (key === 'maxRange') object.maxRange = parseFloat(update[key]);
        else if (key === 'minRange') object.minRange = parseFloat(update[key]);
        ...
        if (key === 'numberOfRays') object.numberOfRays = parseInt(update[key]);
        else if (key === 'aperture') object.aperture = parseFloat(update[key]);
      ...
      if (object instanceof WbLight) {
        if (key === 'on') object.on = update[key].toLowerCase() === 'true';
        else if (key === 'ambientIntensity')
          object.ambientIntensity = parseFloat(update[key]);
        else if (key === 'intensity')
          object.intensity = parseFloat(update[key]);
      } else if (object instanceof WbAbstractCamera) {
        if (key === 'width') object.width = parseInt(update[key]);
    ...
    if (typeof object.parent !== 'undefined') {
      ...
      if (
        ...
        parent.nodeType === WbNodeType.WB_NODE_PROPELLER &&
        parent.currentHelix !== object.id &&
  ...
  processServerMessage(data, view) {
    if (data.startsWith('application/json:')) {
      if (typeof view.time !== 'undefined') {
        ...
        if (document.getElementById('webots-clock'))
          document.getElementById(
            'webots-clock'
          ).innerHTML = webots.parseMillisecondsIntoReadableTime(frame.time);
        ...
        // parse the labels even so the scene loading is not completed
        data = data.substring(data.indexOf(':') + 1);
      ...
      this.loadObject(
        data,
        parentId === '0' ? WbWorld.instance.root.id.replace('n', '') : parentId
      );
      ...
      // Ensure the socket is ready before sending the pause command.
      const waitForSocketReady = () => {
        if (view.stream.socket.readyState === WebSocket.OPEN) {
          view.stream.socket.send('pause');
        } else if (view.stream.socket.readyState === WebSocket.CONNECTING) {
          setTimeout(waitForSocketReady, 100); // Check again after a delay
        } else {
          console.error("WebSocket is in CLOSED or CLOSING state.");
        }
      };

      // Call the function to ensure WebSocket is ready
      waitForSocketReady();
...
function convertStringToVec3Array(string) {
  ...
  if (typeof floatArray !== 'undefined') {
    for (let i = 0; i < floatArray.length; i += 3)
      vecArray.push(
        new WbVector3(floatArray[i], floatArray[i + 1], floatArray[i + 2])
      );
  }
