...
export default class ImageLoader {
  static loadImageTextureInWren(
    imageTexture,
    prefix,
    url,
    checkTransparency = true
  ) {
    const needToDownloadTexture = Module.ccall(
      'wr_texture_2d_copy_from_cache',
      'number',
      ['string'],
      [url]
    );
    if (needToDownloadTexture !== 0) return Promise.resolve();
  ...
  static loadTextureData(prefix, url, isHdr, rotation) {
    ...
    if (url.startsWith('webots://')) {
      ...
      if (
        typeof ImageLoader.branch === 'undefined' ||
        ImageLoader.branch === ''
      )
    ...
    if (typeof prefix !== 'undefined' && !url.startsWith('http')) {
      if (
        ...
        if (typeof worldsPath !== 'undefined')
          worldsPath =
            worldsPath.substring(0, worldsPath.lastIndexOf('/')) + '/';
        else worldsPath = '';
    ...
    if (isHdr) {
      ...
      return this.#loadImage(url).then(img => {
        ...
        if (img.width > 0 && img.height > 0) {
          const dataBGRA = context.getImageData(0, 0, img.width, img.height)
            .data;
    ...
    if (rotate === 90) {
      for (let x = 0; x < image.width; x++) {
        for (let y = 0; y < image.height; y++) {
          ...
          for (let c = 0; c < 3; c++) rotatedbits[u + c] = image.bits[v + c];
        }
      }
      ...
      for (let x = 0; x < image.width; x++) {
        for (let y = 0; y < image.height; y++) {
          const u = y * image.width * 3 + x * 3;
          const v =
            (image.height - 1 - y) * image.width * 3 +
            (image.width - 1 - x) * 3;
          for (let c = 0; c < 3; c++) rotatedbits[u + c] = image.bits[v + c];
        }
      }
