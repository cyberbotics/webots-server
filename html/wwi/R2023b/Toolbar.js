...
export default class Toolbar {
  ...
  constructor(view, type, parentNode) {
    ...
    if (type === 'animation') this.#createAnimationToolbar();
    else if (type === 'scene') this.#createSceneToolbar();
    else if (type === 'streaming') this.#createStreamingToolbar();
    else if (type === 'proto') this.#createProtoToolbar();

    this.#resizeToolbar();
    this.toolbar.style.minWidth = this.minWidth + 'px';

    if (
      SystemInfo.isMobileDevice() &&
      !SystemInfo.isIOS() &&
      !SystemInfo.isSafari()
    ) {
      ...
      if (!screen.orientation || screen.orientation === 'undefined')
        this.#previousScreenOrientation =
          window.orientation % 180 === 0 ? 'portrait' : 'landscape';
      else
        this.#previousScreenOrientation = screen.orientation.type.includes(
          'portrait'
        )
          ? 'portrait'
          : 'landscape';
      ...
      screen.orientation.addEventListener(
        'change',
        this.#mobileOrientationChangeFullscreen.bind(this)
      );

      this.#fullscreenButton.style.animation =
        'animation-scale-up-lg 2s infinite forwards';
      this.#fullscreenButton.addEventListener(
        'click',
        () => this.#removeFullscreenAnimation(this.#fullscreenButton),
        { once: true }
      );
    }
  }
  ...
  #createAnimationToolbar() {
    if (
      ...
      typeof this.#view.animation === 'undefined'
    )
      return;
  ...
  #createSceneToolbar() {
    if (
      (this.type !== 'scene' && this.type !== 'proto') ||
      typeof this.#view === 'undefined'
    )
      return;
  ...
  #createStreamingToolbar() {
    ...
    this.#createInfoButton();
    if (this.#view.mode !== 'mjpeg') this.#createSettings();
    if (!SystemInfo.isIOS() && !SystemInfo.isSafari())
      this.#createFullscreenButtons();
  }
  ...
  #resizeToolbar() {
    this.#toolbarResizeObserver = new ResizeObserver(() => {
      this.#scale =
        this.minWidth > this.parentNode.offsetWidth
          ? this.parentNode.offsetWidth / this.minWidth
          : 1;
  ...
  #mobileOrientationChangeFullscreen() {
    ...
    if (!screen.orientation || screen.orientation === 'undefined')
      ...
      screenOrientation = screen.orientation.type.includes('portrait')
        ? 'portrait'
        : 'landscape';
    if (
      this.#previousScreenOrientation === 'portrait' &&
      screenOrientation === 'landscape' &&
      !isFullscreen()
    )
      requestFullscreen(this.#view);
    else if (
      this.#previousScreenOrientation === 'landscape' &&
  ...
  removeToolbar() {
    if (
      typeof this.#view.mouseEvents !== 'undefined' &&
      typeof this.#view.mouseEvents.hideToolbar
    ) {
  ...
  #createToolbarButton(name, tooltipText, click) {
    ...
    if (typeof click === 'function') button.onclick = () => click();
  ...
  hideToolbar(force) {
    ...
    if (
      typeof this.robotWindowPane !== 'undefined' &&
      this.robotWindowPane.style.visibility === 'visible'
    )
      return;

    if (
      typeof this.worldSelectionPane !== 'undefined' &&
    ...
    let isPlaying = true;
    if (
      document.getElementById('play-button-id') &&
      document.getElementById('play-button-id').className !== 'icon-pause'
    )
      isPlaying = false;

    let settingsPane = true;
    if (
      typeof this.settingsPane !== 'undefined' &&
    ...
    let gtaoPane = true;
    if (
      typeof this.#gtaoPane !== 'undefined' &&
    ...
    if (this.type === 'animation') {
      ...
      let speedPane = true;
      if (
        typeof this.#speedPane !== 'undefined' &&
        this.#speedPane.style.visibility !== 'hidden'
      )
        speedPane = false;

      canHide =
        !isSelected && isPlaying && settingsPane && gtaoPane && speedPane;
    } else if (this.type === 'streaming') {
      if (document.getElementById('run-button'))
        isPlaying =
          isPlaying ||
          document.getElementById('run-button-id').className === 'icon-pause';

      canHide = isPlaying && settingsPane && gtaoPane;
    } else if (this.type === 'scene') canHide = settingsPane && gtaoPane;

    if (canHide) this.toolbar.style.display = 'none';
  }

  showToolbar(force) {
    if (force) this.forceHidden = false;
  ...
  #createPlayButton() {
    ...
    if (this.type === 'animation')
      ...
      if (action === 'pause') this.realTime();
    }

    this.playButton = this.#createToolbarButton('play', 'Play (k)', () =>
    ...
    document.addEventListener(
      'keydown',
      (this.keydownRefK = _ => this.#playKeyboardHandler(_))
    );
    if (
      !(
        typeof this.parentNode.showPlay === 'undefined' ||
        this.parentNode.showPlay
      )
    )
  ...
  #triggerPlayPauseButton() {
    ...
    if (this.type === 'animation' && typeof animation !== 'undefined') {
      if (animation.gui === 'real-time') animation.pause();
      else {
        animation.gui = 'real-time';
        animation.start =
          new Date().getTime() -
          (animation.data.basicTimeStep * animation.step) / animation.speed;
  ...
  #playKeyboardHandler(e) {
    if (e.code === 'KeyK') this.#triggerPlayPauseButton();
  }

  #createTerminalButton() {
    this.terminalButton = this.#createToolbarButton(
      'terminal',
      'Terminal',
      undefined
    );
    ...
    if (
      !(
        typeof this.parentNode.showTerminal === 'undefined' ||
        this.parentNode.showTerminal
      )
    )
      this.terminalButton.style.display = 'none';
    else this.minWidth += 44;
  }
  ...
  #createTerminal() {
    ...
    const terminalPositionY =
      (this.parentNode.offsetHeight - terminalHeight) / 2;

    this.terminal.floatingWindow.addEventListener('mouseover', () =>
      this.showToolbar()
    );
    this.terminal.headerQuit.addEventListener('mouseup', _ =>
      this.#changeFloatingWindowVisibility(this.terminal.getId())
    );
  ...
  #createIdeButton() {
    ...
    if (this.#view.mobileDevice && Math.min(screen.height, screen.width) < 700)
      this.#view.ide = false;
    if (!this.#view.ide) return;

    this.ideButton = this.#createToolbarButton(
      'ide',
      'Source code editor',
      undefined
    );
    ...
    if (
      !(
        typeof this.parentNode.showIde === 'undefined' ||
        this.parentNode.showIde
      )
    )
  ...
  #createIde() {
    ...
    this.ideWindow.floatingWindow.addEventListener('mouseover', () =>
      this.showToolbar()
    );
    this.ideWindow.headerQuit.addEventListener('mouseup', _ =>
      this.#changeFloatingWindowVisibility(this.ideWindow.getId())
    );

    this.ideWindow.setSize(ideWidth, ideHeight);
    this.ideWindow.setPosition(idePositionX, idePositionY);
    this.ideButton.onclick = () =>
      this.#changeFloatingWindowVisibility(this.ideWindow.getId());
  ...
  #createRobotWindowButton() {
    if (this.#view.robots.length > 0) {
      this.robotWindowButton = this.#createToolbarButton(
        'robot-window',
        'Robot windows (w)'
      );
      this.toolbarRight.appendChild(this.robotWindowButton);
      this.robotWindowButton.addEventListener(
        'mouseup',
        (this.mouseupRefWFirst = _ => this.#showAllRobotWindows()),
        { once: true }
      );
      document.addEventListener(
        'keydown',
        (this.keydownRefWFirst = _ =>
          this.#robotWindowPaneKeyboardHandler(_, true)),
        { once: true }
      );
      this.keydownRefW = undefined;
      window.addEventListener('click', _ =>
        this.#closeRobotWindowPaneOnClick(_)
      );
      if (
        !(
          typeof this.parentNode.showRobotWindow === 'undefined' ||
          this.parentNode.showRobotWindow
        )
      )
  ...
  #closeRobotWindowPaneOnClick(event) {
    if (typeof this.robotWindowPane !== 'undefined') {
      if (
        event.srcElement.id !== 'robot-window-button' &&
        this.robotWindowPane.style.visibility === 'visible'
      ) {
        if (
          !(
            event.srcElement.id.startsWith('close-') ||
            event.srcElement.id.startsWith('enable-robot-window')
          )
        )
  ...
  #createRobotWindows() {
    ...
    this.#view.robots.forEach(robot => {
      if (robot.window !== '<none>') {
        ...
        let robotWindow = new FloatingRobotWindow(
          this.parentNode,
          robot.name,
          robotWindowUrl,
          robot.window,
        ...
        if (robot.visible)
          // if the robot window is visible by default in Webots, then show it
          robotWindow.changeVisibility();
        if (mainWindow) this.robotWindows.unshift(robotWindow);
        else this.robotWindows.push(robotWindow);
    ...
    this.robotWindows.forEach(rw => {
      ...
      rw.headerQuit.addEventListener('mouseup', _ =>
      ...
      if (rw.isMainWindow) {
        ...
        rw.setPosition(
          viewWidth -
            robotWindowWidth -
            margin -
            numCol * (margin + robotWindowWidth) +
            (layer * margin) / 3,
  ...
  #initializeWindowLayerChanges() {
    window.addEventListener('blur', _ => {
      ...
      if (
        newElement.parentNode.parentNode.classList &&
        newElement.parentNode.parentNode.classList.contains('floating-window')
      ) {
        document.querySelectorAll('.floating-window').forEach(fw => {
          fw.style.zIndex = '3';
        });
        document.getElementById(
          newElement.parentNode.parentNode.id
        ).style.zIndex = '4';
      }
    });
  }
  ...
  #refreshRobotWindowContent() {
    this.robotWindows.forEach(rw => {
      if (
        typeof document.getElementById(rw.name + '-robot-window').src !==
        'undefined'
      )
        document.getElementById(
          rw.name + '-robot-window'
        ).src = document.getElementById(rw.name + '-robot-window').src;
    });
  }
  ...
  removeRobotWindows() {
    ...
    if (typeof this.robotWindows !== 'undefined')
      document
        .querySelectorAll('.floating-robot-window')
        .forEach(fw => fw.remove());
  }

  #changeRobotWindowPaneVisibility(event) {
    if (this.robotWindowPane.style.visibility === 'hidden')
      this.robotWindowPane.style.visibility = 'visible';
    else this.robotWindowPane.style.visibility = 'hidden';
  }

  #robotWindowPaneKeyboardHandler(e, isFirst) {
    if (e.code === 'KeyW') {
      if (isFirst) this.#showAllRobotWindows();
      else this.#changeRobotWindowPaneVisibility(e);
    }
  }
  ...
  #showAllRobotWindows() {
    ...
    this.robotWindowButton.removeEventListener(
      'mouseup',
      this.mouseupRefWFirst
    );
    ...
    this.robotWindowButton.addEventListener('mouseup', _ =>
      this.#changeRobotWindowPaneVisibility(_)
    );
    document.addEventListener(
      'keydown',
      (this.keydownRefW = _ => this.#robotWindowPaneKeyboardHandler(_, false))
    );
  }
    ...
    const resizeObserver = new ResizeObserver(() => {
      ...
      floatingWindows.forEach(fw => {
        const maxLeft =
          ...
          parseInt(window.getComputedStyle(fw).minWidth);
        const maxTop =
          this.parentNode.offsetHeight -
          parseInt(window.getComputedStyle(fw).minHeight);
        fw.style.left =
          (fw.offsetLeft > maxLeft ? maxLeft : fw.offsetLeft) + 'px';
        ...
        fw.style.width =
          (fw.offsetWidth > maxWidth ? maxWidth : fw.offsetWidth) + 'px';
        fw.style.height =
          (fw.offsetHeight > maxHeight ? maxHeight : fw.offsetHeight) + 'px';
      });
    });
  ...
  #createInfoButton() {
    this.infoButton = this.#createToolbarButton(
      'info',
    ...
    if (
      !(
        typeof this.parentNode.showInfo === 'undefined' ||
        this.parentNode.showInfo
      )
    )
  ...
  #createInformation() {
    ...
    if (
      typeof WbWorld.instance !== 'undefined' &&
  ...
  #displayInformationWindow() {
    if (typeof this.informationPanel !== 'undefined') {
      if (this.informationPanel.informationPanel.style.display === 'block')
        ...
        const infoScaleWidth =
          this.informationPanel.informationPanel.offsetWidth >
          0.8 * this.parentNode.offsetWidth
            ? (0.8 * this.parentNode.offsetWidth) /
              this.informationPanel.informationPanel.offsetWidth
            : 1.0;
        const infoScaleHeight =
          this.informationPanel.informationPanel.offsetHeight >
          0.8 * this.parentNode.offsetHeight
            ? (0.8 * this.parentNode.offsetHeight) /
              this.informationPanel.informationPanel.offsetHeight
            : 1.0;
        this.informationPanel.informationPanel.style.transform =
          'scale(' + Math.min(infoScaleHeight, infoScaleWidth) + ')';
      }
    }
  }
  ...
  #closeInfoOnClick(event) {
    if (
      typeof this.informationPanel !== 'undefined' &&
      !this.informationPanel.informationPanel.contains(event.target) &&
  ...
  #createSettings() {
    this.toolbarRight.appendChild(
      this.#createToolbarButton('settings', 'Settings')
    );
    this.minWidth += 44;
  ...
  #createSettingsPane() {
    ...
    document.addEventListener(
      'mouseup',
      (this.settingsRef = _ => this.#changeSettingsPaneVisibility(_))
    );
    ...
    if (this.type === 'animation') this.#createChangeSpeed();
  }

  #changeSettingsPaneVisibility(event) {
    // Avoid to close the settings when modifying the shadows or the other options
    if (
      event.srcElement.id === 'enable-shadows' ||
      event.srcElement.id === 'enable-follow' ||
    ...
    if (this.type === 'animation') {
      if (
        event.srcElement.id === 'playback-li' ||
        typeof this.#speedPane === 'undefined'
      )
        return;
    ...
    if (
      event.target.id === 'settings-button' &&
      this.settingsPane.style.visibility === 'hidden' &&
      this.#gtaoPane.style.visibility === 'hidden' &&
      speedPanelHidden
    )
    ...
    this.#gtaoPane.style.visibility = 'hidden';
    if (this.type === 'animation') this.#speedPane.style.visibility = 'hidden';
  }
  ...
  #createGtaoPane() {
    ...
    for (let i of ['Low', 'Medium', 'High', 'Ultra']) {
      ...
      gtaoLevelLi.id = i;
      label = document.createElement('span');
      if (this.gtaoLevelToText(GtaoLevel) === i) label.innerHTML = '&check;';
      label.id = 'c-' + i;
  ...
  changeGtao(event) {
    ...
    if (gtaoLabel) gtaoLabel.innerHTML = event.srcElement.id;
    this.settingsPane.style.visibility = 'visible';
    for (let i of document.getElementsByClassName('check-gtao')) {
      if (i.id === 'c-' + event.srcElement.id) i.innerHTML = '&check;';
      else i.innerHTML = '';
    }
    const animation = this.#view.animation;
    if (animation)
  ...
  #createFullscreenButtons() {
    ...
    this.#exitFullscreenButton = this.#createToolbarButton(
      'windowed',
      'Exit full screen (f)',
      () => exitFullscreen()
    );
    ...
    document.addEventListener(
      'keydown',
      (this.keydownRefF = _ => this.#fullscrenKeyboardHandler(_))
    );
  }
  ...
  #createSlider() {
    ...
    document.addEventListener(
      'sliderchange',
      (this.sliderchangeRef = _ => this.#updateSlider(_))
    );
    this.toolbar.appendChild(this.#timeSlider);
    this.#timeSlider.shadowRoot
      .getElementById('range')
      .addEventListener(
        'mousemove',
        (this.updateFloatingTimeRef = _ => this.#updateFloatingTimePosition(_))
      );
    this.#timeSlider.shadowRoot
      .getElementById('range')
      .addEventListener(
        'mouseleave',
        (this.hideFloatingTimeRef = _ => this.#hideFloatingTimePosition(_))
      );
  }
  ...
  #updateSlider(event) {
    ...
    const requestedStep = Math.floor(
      (animation.data.frames.length * clampedValued) / 100
    );
    animation.start =
      new Date().getTime() -
      Math.floor(animation.data.basicTimeStep * animation.step);
    animation.updateAnimationState(requestedStep);

    this.#timeSlider.setTime(
      this.#formatTime(animation.data.frames[requestedStep].time)
    );
  }

  #updateFloatingTimePosition(e) {
    this.#timeSlider.shadowRoot.getElementById(
      'floating-time'
    ).style.visibility = 'visible';

    const bounds = this.#timeSlider.shadowRoot
      .getElementById('range')
    ...
    const requestedStep = Math.floor(
      (this.#view.animation.data.frames.length * clampedValued) / 100
    );
    this.#timeSlider.setTime(
      this.#formatTime(this.#view.animation.data.frames[requestedStep].time)
    );
  ...
  #createAnimationTimeIndicator() {
    ...
    this.#currentTime.innerHTML = this.#formatTime(
      this.#view.animation.data.frames[0].time
    );
    ...
    const time = this.#formatTime(
      this.#view.animation.data.frames[
        this.#view.animation.data.frames.length - 1
      ].time
    );
  ...
  #createCustomWindowButton() {
    this.customWindowButton = this.#createToolbarButton(
      'custom-window',
      'Custom window',
      undefined
    );
  ...
  #createCustomWindow() {
    ...
    const customWindowPositionX =
      (this.parentNode.offsetWidth - customWindowWidth) / 2;
    const customWindowPositionY =
      (this.parentNode.offsetHeight - customWindowHeight) / 2;

    this.customWindow.floatingWindow.addEventListener('mouseover', () =>
      this.showToolbar()
    );
    this.customWindow.headerQuit.addEventListener('mouseup', _ =>
      this.#changeFloatingWindowVisibility(this.customWindow.getId())
    );
    ...
    this.customWindowButton.onclick = () =>
      this.#changeFloatingWindowVisibility(this.customWindow.getId());
  ...
  #formatTime(time) {
    if (typeof this.#unusedPrefix === 'undefined') {
      const maxTime = this.#view.animation.data.frames[
        this.#view.animation.data.frames.length - 1
      ].time;
      if (maxTime < 60000) this.#unusedPrefix = 6;
      else if (maxTime < 600000) this.#unusedPrefix = 4;
      else if (maxTime < 3600000) this.#unusedPrefix = 3;
      else if (maxTime < 36000000) this.#unusedPrefix = 1;
    }

    return this.#parseMillisecondsIntoReadableTime(time).substring(
      this.#unusedPrefix
    );
  }
  ...
  #createSpeedPane() {
    ...
    for (let i of ['0.25', '0.5', '0.75', '1', '1.25', '1.5', '1.75', '2']) {
      ...
      playbackLi.id = i;
      label = document.createElement('span');
      if (i === '1') label.innerHTML = '&check;';
      label.id = 'c' + i;
      label.className = 'check-speed';
      playbackLi.appendChild(label);
      label = document.createElement('span');
      if (i === '1') label.innerHTML = 'Normal';
      else label.innerHTML = i;
      label.className = 'setting-span';
      playbackLi.appendChild(label);
      label = document.createElement('div');
      label.className = 'spacer';
    ...
    if (animation) {
      ...
      if (speedDisplay)
        speedDisplay.innerHTML =
          animation.speed === '1' ? 'Normal' : animation.speed;
      this.settingsPane.style.visibility = 'visible';
      for (let i of document.getElementsByClassName('check-speed')) {
        if (i.id === 'c' + animation.speed) i.innerHTML = '&check;';
        else i.innerHTML = '';
      }
      animation.start =
        new Date().getTime() -
  ...
  removeAnimationToolbar() {
    ...
    if (typeof this.#timeSlider !== 'undefined') {
      this.#timeSlider.shadowRoot
        .getElementById('range')
        .removeEventListener('mousemove', this.updateFloatingTimeRef);
      this.updateFloatingTimeRef = undefined;
      this.#timeSlider.shadowRoot
        .getElementById('range')
        .removeEventListener('mouseleave', this.hideFloatingTimeRef);
  ...
  _createRestoreViewpointButton() {
    this.toolbarRight.appendChild(
      this.#createToolbarButton('reset-scene', 'Reset the Scene', () =>
        this.#resetViewpoint()
      )
    );
    this.minWidth += 44;
  ...
  #createQuitButton() {
    const quitButton = this.#createToolbarButton(
      'quit',
      'Quit the simulation',
    ...
    if (
      !(
        typeof this.parentNode.showQuit === 'undefined' ||
        this.parentNode.showQuit
      )
    )
  ...
  #createReloadButton() {
    const reloadButton = this.#createToolbarButton(
      'reload',
    ...
    if (!this.parentNode.showReload) reloadButton.style.display = 'none';
    else this.minWidth += 44;
  ...
  reset(reload = false) {
    ...
    if (
      typeof this.pauseButton !== 'undefined' &&
      this.playButtonElement.className === 'icon-pause'
    )
      this.#view.currentState = 'real-time';
    else if (
      typeof this.runButton !== 'undefined' &&
    ...
    if (reload) this.#view.stream.socket.send('reload');
    else this.#view.stream.socket.send('reset');

    if (this.robotWindows) this.#refreshRobotWindowContent();
  }
  ...
  #createResetButton() {
    let resetButton = this.#createToolbarButton(
      'reset',
    ...
    if (
      !(
        typeof this.parentNode.showReset === 'undefined' ||
        this.parentNode.showReset
      )
    )
  ...
  #createStepButton() {
    let stepButton = this.#createToolbarButton(
      'step',
    ...
    if (
      !(
        typeof this.parentNode.showStep === 'undefined' ||
        this.parentNode.showStep
      )
    )
      stepButton.style.display = 'none';
    else this.minWidth += 32;
  }
  ...
  #createRunButton() {
    this.runButton = this.#createToolbarButton('run', 'Run', () =>
      this.#triggerRunPauseButton()
    );
    this.runButtonElement = this.runButton.childNodes[0];
    this.runButtonElement.title = this.runButton.childNodes[1];
    if (!this.parentNode.showRun) this.runButton.style.display = 'none';
    else this.minWidth += 44;
    if (
      this.#view.currentState === 'run' ||
  ...
  #createWorldSelectionButton() {
    this.worldSelectionButton = this.#createToolbarButton(
      'world-selection',
      'Select world'
    );
    this.toolbarLeft.appendChild(this.worldSelectionButton);
    this.createWorldSelectionPane();
    this.worldSelectionButton.addEventListener('mouseup', _ =>
      this.#changeWorldSelectionPaneVisibility(_)
    );
    window.addEventListener('click', _ =>
      this.#closeWorldSelectionPaneOnClick(_)
    );

    if (
      !(
        typeof this.parentNode.showWorldSelection === 'undefined' ||
        this.parentNode.showWorldSelection
      ) ||
  ...
  createWorldSelectionPane() {
    ...
    this.worldSelectionPane.addEventListener('mouseover', () =>
      this.showToolbar()
    );
  ...
  #changeWorld(name) {
    if (this.#view.currentWorld === name) return;

    this.worldSelectionPane.style.visibility = 'hidden';
    document.getElementById(
      this.#view.currentWorld + '-button'
    ).checked = false;
    document.getElementById(name + '-button').checked = true;

    if (this.#view.broadcast || typeof name === 'undefined') return;
    this.#view.progress.setProgressBar('block', 'Loading ' + name + '...');
    this.hideToolbar(true);
    let previousOnready = this.#view.onready;
    let stateBeforeChange = this.#view.currentState;
    this.#view.onready = () => {
      if (previousOnready === 'function') previousOnready();
      this.showToolbar(true);
      if (stateBeforeChange === 'real-time') this.realTime();
  ...
  #changeWorldSelectionPaneVisibility(event) {
    if (this.worldSelectionPane.style.visibility === 'hidden')
      this.worldSelectionPane.style.visibility = 'visible';
    else this.worldSelectionPane.style.visibility = 'hidden';
  }

  #closeWorldSelectionPaneOnClick(event) {
    if (
      event.srcElement.id !== 'world-selection-button' &&
      this.worldSelectionPane.style.visibility === 'visible'
    ) {
  ...
  #createProtoParameterButton() {
    this.protoParameterButton = this.#createToolbarButton(
      'proto-parameter',
      'Proto parameter window',
      undefined
    );
  ...
  #createProtoParameterWindow() {
    this.protoParameterWindow = new FloatingProtoParameterWindow(
      this.parentNode,
      this.parentNode.protoManager,
      this.#view
    );

    this.protoParameterWindow.floatingWindow.addEventListener('mouseover', () =>
      this.showToolbar()
    );
    this.protoParameterWindow.headerQuit.addEventListener('mouseup', _ =>
      this.#changeFloatingWindowVisibility(this.protoParameterWindow.getId())
    );

    this.protoParameterButton.onclick = () =>
      this.#changeFloatingWindowVisibility(this.protoParameterWindow.getId());
  ...
  protoParameterWindowInitializeSizeAndPosition() {
    ...
    const protoParameterWindowPositionX =
      this.parentNode.offsetWidth - protoParameterWindowWidth;
    const protoParameterWindowPositionY =
      (this.parentNode.offsetHeight - 40 - protoParameterWindowHeight) / 2;
    this.protoParameterWindow.setPosition(
      protoParameterWindowPositionX,
      protoParameterWindowPositionY
    );

    this.protoParameterWindow.setSize(
      protoParameterWindowWidth,
      protoParameterWindowHeight
    );
  }
}
