...
export default class Server {
  ...
  constructor(url, view, onready) {
    ...
    } else this.#repository = this.#url.substring(n + 5);
  }
  ...
  onOpen(event) {
    ...
    if (this.#view.mode === 'mjpeg') message += ',"mode":"mjpeg"';
    message += '}}';
  ...
  onMessage(event) {
    ...
    if (
      message.indexOf('webots:ws://') === 0 ||
      message.indexOf('webots:wss://') === 0
    ) {
      ...
      console.log(
        'The server is saturated. Queue to wait: ' +
          message.substring(6) +
          ' client(s).'
      );
    else if (message === '.') {
      // received every 5 seconds when Webots is running
      // nothing to do
      ...
      else if (body.startsWith('Creating network')) percent = 20;
      else if (body.startsWith('Step '))
        percent =
          20 + (65 * parseInt(body.charAt(5))) / (parseInt(body.charAt(7)) + 1);
      else if (body.endsWith('done')) percent = 85;
      else if (body.startsWith('webots')) percent = 90;
      this.#view.progress.setProgressBar(
        'block',
        'same',
        5 + 0.6 * percent,
    ...
    } else if (message.indexOf('ide: ') === 0) this.#view.ide = true;
      ...
      if (shutdownTimeout > 0)
        this.timeout = setTimeout(() => {
          alert(
            'Warning: the time limit is almost reached.\nThe simulation will be automatically closed in 5 minutes'
          );
        }, shutdownTimeout * 1000);
    } else
      console.log(
        'Received an unknown message from the Webots server socket: "' +
          message +
          '"'
      );
  }
