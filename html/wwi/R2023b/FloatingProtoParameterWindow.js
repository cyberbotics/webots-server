...
export default class FloatingProtoParameterWindow extends FloatingWindow {
  ...
  populateProtoParameterWindow() {
    ...
    if (contentDiv) {
      ...
      for (let key of keys) {
        ...
        if (
          parameter.restrictions.length > 0 &&
          !this.unsupportedRestrictions.includes(parameter.type)
        )
        ...
        else this.#createMFField(key, contentDiv);
      ...
      if (this.node.isRoot) {
        ...
        this.backBuffer = [];
      } else this.#createBackButton(contentDiv);
  ...
  #refreshParameterRow(parameter, mfId, isCreatingParameters) {
    ...
    if (
      (parameter.isTemplateRegenerator ||
        parameter.value instanceof SFNode ||
        parameter.value instanceof MFNode) &&
      !isCreatingParameters
    )
      this.updateDevicesTabs();

    if (parameter.isDefault()) this.#disableResetButton(resetButton);
    else this.#enableResetButton(resetButton);

    if (parameter.value instanceof SFNode) {
      const currentNodeButton = document.getElementById(
        'current-node-' + parameter.name
      );
      const deleteNodeButton = document.getElementById(
        'delete-node-' + parameter.name
      );
      const configureNodeButton = document.getElementById(
        'configure-node-' + parameter.name
      );
      ...
      if (parameter.value.value === null) {
        ...
        configureNodeButton.title =
          'Configure ' + parameter.value.value.name + ' node';
    ...
    if (parameter.value instanceof MFNode) {
      const nodes = document.getElementsByClassName(
        'value-parameter mf-parameter mf-id-' + mfId
      );
    ...
    if (
      parameter.restrictions.length > 0 &&
      ![VRML.SFNode, VRML.MFNode].includes(parameter.type)
    ) {
  ...
  #createSFVec3Field(key, parent) {
    ...
    p.inputs.push(
      this.#createVectorInput('y', parameter.value.value.y, values, () =>
        this.#vector3OnChange(p)
      )
    );
    p.inputs.push(
      this.#createVectorInput('z', parameter.value.value.z, values, () =>
        this.#vector3OnChange(p)
      )
    );
  ...
  #createSFColorField(key, parent) {
    ...
    resetButton.onclick = () => {
      ...
      input.value = this.rgbToHex(
        parseInt(defaultValue.r * 255),
        parseInt(initialValue.g * 255),
  ...
  rgbToHex(r, g, b) {
    return (
      '#' +
      this.#colorComponentToHex(r) +
      this.#colorComponentToHex(g) +
      this.#colorComponentToHex(b)
    );
  }
  ...
  #createMFField(key, parent) {
    ...
    resetButton.onclick = () => {
      const nodesToRemove = document.getElementsByClassName(
        'mf-id-' + currentMfId
      );
      let maxRowNumber = 0;
      for (let i = nodesToRemove.length - 1; i >= 0; i--) {
        const rowNumber = this.#getRow(nodesToRemove[i]);
        if (rowNumber > maxRowNumber) maxRowNumber = rowNumber;
        nodesToRemove[i].parentNode.removeChild(nodesToRemove[i]);
      }

      const nodeModel = parameter.node.model;
      const parameterModel =
        nodeModel[parameter.node.isProto ? 'parameters' : 'fields'][
          parameter.name
        ]['defaultValue'];
      ...
      const maxRowNumberNeeded =
        parameter.value.value.length * 2 + 1 + resetButtonRow;

      // Need to offset the following rows by the difference to keep the coherency.
      if (maxRowNumber > maxRowNumberNeeded)
        this.#offsetNegativelyRows(
          resetButtonRow,
          maxRowNumber - maxRowNumberNeeded
        );
      else if (maxRowNumber < maxRowNumberNeeded)
        this.#offsetPositivelyRows(
          resetButtonRow + 1,
          maxRowNumberNeeded - maxRowNumber
        );

      this.#populateMFField(
        resetButton,
        parent,
        parameter,
        resetButtonRow,
        currentMfId,
        !hideShowButton.isHidden
      );

      this.#MFOnChange(
        'value-parameter mf-parameter mf-id-' + currentMfId,
        parameter
      );
    };

    this.#rowNumber += this.#populateMFField(
      resetButton,
      parent,
      parameter,
      this.#rowNumber,
  ...
  #MFOnChange(className, parameter) {
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      if (parameter.type === VRML.MFVec3f) {
        value = {
          x: this.#sanitizeNumber(
            elements[i].childNodes[0].childNodes[0].value
          ),
          y: this.#sanitizeNumber(
            elements[i].childNodes[1].childNodes[0].value
          ),
          z: this.#sanitizeNumber(
            elements[i].childNodes[2].childNodes[0].value
          ),
        };
        ...
        parameter.type === VRML.MFFloat ||
        parameter.type === VRML.MFInt32
      )
        ...
        value = {
          ...
          a: this.#sanitizeNumber(
            elements[i].childNodes[3].childNodes[0].value
          ),
        };
    ...
    const array = [];
    let i = 0;
    for (let value of valuesMap.values()) array[i++] = value;
  ...
  #populateMFField(resetButton, parent, parameter, firstRow, mfId, isVisible) {
    ...
    for (let i = 0; i < parameter.value.value.length; i++) {
      ...
      if (parameter.type === VRML.MFNode) {
        this.#createMFNodeRow(
          parameter.value.value[i].value,
  ...
  #createMfRow(value, row, parent, mfId, resetButton, parameter, isVisible) {
    ...
    if (parameter.type === VRML.MFFloat || parameter.type === VRML.MFInt32) {
      ...
      input.type = 'color';
      input.value = this.rgbToHex(
        parseInt(value.r * 255),
        parseInt(value.g * 255),
        parseInt(value.b * 255)
      );
    ...
    this.#createRemoveMFButton(p, () =>
      this.#MFOnChange(p.className, parameter)
    );

    // Add row
    const addRow = this.#createAddRowSection(
      mfId,
      resetButton,
      row,
      parent,
  ...
  #createMFNodeField(key, parent) {
    ...
    resetButton.onclick = () => {
      ...
      const parameterModel =
        protoModel[parameter.node.isProto ? 'parameters' : 'fields'][
          parameter.name
        ]['defaultValue'];
    ...
    this.#rowNumber += this.#populateMFNode(
      resetButton,
      parent,
      parameter,
      this.#rowNumber,
  ...
  #createMFNodeRow(
    ...
    currentNodeButton.onclick = async () => {
      if (typeof this.nodeSelector === 'undefined')
        this.nodeSelector = new NodeSelectorWindow(
          this.parentNode,
          this.#protoManager.proto
        );

      this.nodeSelector.show(
        parameter,
        p,
        this.#MFNodeOnChange.bind(this),
        parent,
        mfId,
        resetButton
      );
    ...
    configureNodeButton.onclick = async () => {
      ...
      if (typeof index === 'undefined')
        throw new Error(
          'The PROTO node to be configured is not defined, this should never be the case.'
        );
    ...
    this.#createRemoveMFButton(p, () =>
      this.#MFNodeOnRemoval(parameter, undefined, p, undefined, mfId)
    );
  ...
  async #MFNodeOnInsertion(parameter, url, element, parent, mfId, resetButton) {
    ...
    const newRows = this.#createMFNodeRow(
      node,
      row,
      parent,
      mfId,
  ...
  #rowToParameterIndex(element, mfId) {
    ...
    for (let i = 0; i < nodes.length; ++i) {
      if (this.#getRow(nodes[i]) < minRow) minRow = this.#getRow(nodes[i]);
    }

    const row = this.#getRow(element) - minRow + 2;
    if (row % 2 === 0) return row * 0.5 - 1;
    else return (row - 1) * 0.5 - 1;
  }
  ...
  #createAddRowSection(mfId, resetButton, row, parent, parameter, isVisible) {
    ...
    addRow.onclick = async () => {
      if (parameter.type === VRML.MFNode) {
        ...
        this.nodeSelector.show(
          parameter,
          addRow,
          this.#MFNodeOnInsertion.bind(this),
          parent,
          mfId,
          resetButton
        );
        ...
        else if (parameter.type === VRML.MFVec2f) defaultValue = { x: 0, y: 0 };
        else if (parameter.type === VRML.MFString) defaultValue = '';
        else if (
          parameter.type === VRML.MFFloat ||
          parameter.type === VRML.MFInt32
        )
          defaultValue = 0;
        else if (parameter.type === VRML.MFBool) defaultValue = false;
        else if (parameter.type === VRML.MFRotation)
          defaultValue = { x: 0, y: 0, z: 1, a: 0 };
        else if (parameter.type === VRML.MFColor)
          defaultValue = { r: 0, g: 0, b: 0 };

        const newRows = this.#createMfRow(
          defaultValue,
          row,
          parent,
          mfId,
    ...
    if (isVisible) addRow.style.display = 'flex';
  ...
  #createSFRotation(key, parent) {
    ...
    p.inputs.push(
      this.#createVectorInput('a', parameter.value.value.a, values, () =>
        this.#rotationOnChange(p)
      )
    );
  ...
  #createVectorInput(name, initialValue, parent, callback, first) {
    ...
    span.title = name;
    if (!first) span.style.paddingLeft = '5px';
    const input = document.createElement('input');
    input.type = 'number';
  ...
  #createRestrictedField(key, parent) {
    ...
    select.onchange = e => {
      ...
      parameter.setValueFromJavaScript(
        this.#view,
        parameter.restrictions[selectionIndex].toJS(false)
      );
      this.#refreshParameterRow(parameter);
    };
    ...
    resetButton.onclick = () => {
      ...
      parameter.setValueFromJavaScript(
        this.#view,
        JSON.parse(JSON.stringify(parameter.defaultValue.value))
      );
      this.#refreshParameterRow(parameter);
    };
  ...
  #createSFNodeField(key, parent) {
    ...
    configureNodeButton.title = 'Edit node.';
    configureNodeButton.onclick = async () => {
      if (parameter.value.value === null) return;
  ...
  #hideNodeSelector(event) {
    if (
      typeof this.nodeSelector !== 'undefined' &&
      !this.nodeSelector.nodeSelector.contains(event.target)
    ) {
  ...
  #floatOnChange(node) {
    node.parameter.setValueFromJavaScript(
      this.#view,
      this.#sanitizeNumber(node.input.value)
    );
    this.#refreshParameterRow(node.parameter);
  }
  ...
  #changeBoolText(boolText, input) {
    if (input.checked) boolText.innerHTML = 'TRUE';
    else boolText.innerHTML = 'FALSE';
  }
  ...
  #createCheckbox(parent, key) {
    ...
    exportCheckbox.style.gridRow =
      '' + this.#rowNumber + ' / ' + this.#rowNumber;
    exportCheckbox.style.gridColumn = '1 / 1';
    exportCheckbox.onchange = event =>
      this.fieldsToExport.set(exportCheckbox.key, exportCheckbox.checked);
    ...
    for (const key of keys) {
      ...
      if (device instanceof WbDevice) {
        ...
        div.addEventListener('mouseover', () =>
          this.#displayOptionalRendering(device.id)
        );
        div.addEventListener('mouseleave', () =>
          this.#hideOptionalRendering(device.id)
        );
        div.addEventListener('click', _ =>
          this.#changeVisibility(device.id, _)
        );
  ...
  #appendDevices() {
    ...
    for (const key of this.devicesList.keys()) {
      ...
      if (values.length > 0) {
        ...
        for (const value of values) this.devices.appendChild(value);
      }
    }
  }
  ...
  #displayOptionalRendering(id) {
    if (WbWorld.instance.readyForUpdates) {
      ...
      if (node) node.applyOptionalRendering(true);
  ...
  #hideOptionalRendering(id) {
    const node = WbWorld.instance.nodes?.get(id);
    if (node) {
      if (node.optionnalRenderingLocked) return;
  ...
  populateJointTab() {
    ...
    for (const key of keys) {
      ...
      if (joint instanceof WbJoint) {
        ...
        const endPointName = joint.solidEndPoint()
          ? joint.solidEndPoint().name
          : numberOfJoint;
        let jointType;
        ...
        else jointType = 'Sliderjoint: ';

        div.appendChild(
          this.#createJointInfo(jointType, endPointName, joint.device)
        );
        const parameters = joint.jointParameters;
        this.#createSlider(parameters, joint.device, div, value => {
          if (parameters) parameters.position = value;
          else joint.position = value;
          this.#view.x3dScene.render();
        });
        ...
        if (joint.nodeType === WbNodeType.WB_NODE_BALL_JOINT) {
          ...
          div.appendChild(
            this.#createJointInfo('BallJoint 2: ', endPointName, joint.device2)
          );
          const parameters2 = joint.jointParameters2;
          this.#createSlider(parameters2, joint.device2, div, value => {
            if (parameters2) parameters2.position = value;
            else joint.position2 = value;
            this.#view.x3dScene.render();
          });
          this.joints.appendChild(div);

          div.appendChild(
            this.#createJointInfo('BallJoint 3: ', endPointName, joint.device3)
          );
          const parameters3 = joint.jointParameters3;
          this.#createSlider(parameters3, joint.device3, div, value => {
            if (parameters3) parameters3.position = value;
            else joint.position3 = value;
            this.#view.x3dScene.render();
          });
          this.joints.appendChild(div);
        } else if (joint.nodeType === WbNodeType.WB_NODE_HINGE_2_JOINT) {
        ...
        } else helixName = 'propeller';

        div.appendChild(
          this.#createJointInfo('Propeller: ', helixName, joint.device)
        );
        if (helix) {
          this.#createSlider(undefined, joint.device, div, value => {
            helix.rotation = new WbVector4(
              helix.rotation.x,
              helix.rotation.y,
              helix.rotation.z,
              value
      ...
      open.onclick = () => {
        ...
        if (open.isHidden) {
          ...
          for (const child of children) {
            if (child.style.gridRow[0] > 1) child.style.display = 'flex';
          }
        } else {
          ...
          open.title = 'Show more information';
          for (const child of children) {
            if (child.style.gridRow[0] > 1) child.style.display = 'none';
          }
        }
      };
    } else this.#createGridFiller(grid, 1);
  ...
  #createSlider(parameters, device, parent, callback) {
    ...
    if (
      typeof motor !== 'undefined' &&
      motor.minPosition !== motor.maxPosition
    ) {
      minLabel.innerHTML =
        this.#decimalCount(motor.minPosition) > 2
          ? motor.minPosition.toFixed(2)
          : motor.minPosition;
      slider.min = motor.minPosition;
      maxLabel.innerHTML =
        this.#decimalCount(motor.maxPosition) > 2
          ? motor.maxPosition.toFixed(2)
          : motor.maxPosition;
      slider.max = motor.maxPosition;
    } else if (
      typeof parameters !== 'undefined' &&
      parameters.minStop !== parameters.maxStop
    ) {
      minLabel.innerHTML =
        this.#decimalCount(parameters.minStop) > 2
          ? parameters.minStop.toFixed(2)
          : parameters.minStop;
      slider.min = parameters.minStop;
      maxLabel.innerHTML =
        this.#decimalCount(parameters.maxStop) > 2
          ? parameters.maxStop.toFixed(2)
          : parameters.maxStop;
    ...
    if (typeof parameters === 'undefined') slider.value = 0;
    else slider.value = parameters.position;

    slider.addEventListener('input', _ => {
      if (typeof callback === 'function') callback(_.target.value);
      ...
      if (slider.motorName?.includes('::')) {
        ...
        for (let i = 0; i < this.coupledSlidersList.length; i++) {
          if (
            this.coupledSlidersList[i].motorName.startsWith(coupledMotorName)
          ) {
            const newValue =
              (slider.value * this.coupledSlidersList[i].multiplier) /
              slider.multiplier;
  ...
  #decimalCount(number) {
    ...
    if (numberString.includes('.')) return numberString.split('.')[1].length;
    return 0;
  }
