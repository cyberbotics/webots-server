...
export default class Parser {
  ...
  async parse(text, renderer, finalize = true, parentNode, callback) {
    webots.currentView.progress.setProgressBar(
      'same',
      'Parsing...',
      0,
      'Parsing object...'
    ...
    if (typeof xml === 'undefined') console.error('File to parse not found');
    else {
      ...
      if (typeof head !== 'undefined') {
        for (const child of head.children) {
          if (getNodeAttribute(child, 'name', '') === 'version') {
            if (typeof WbWorld.instance === 'undefined') WbWorld.init();
    ...
    return Promise.all(this.#promises).then(() => {
      ...
      if (
        typeof this.smaaAreaTexture !== 'undefined' &&
        typeof this.smaaSearchTexture !== 'undefined' &&
        typeof this.gtaoNoiseTexture !== 'undefined'
      ) {
        WbWrenPostProcessingEffects.loadResources(
          this.smaaAreaTexture,
          this.smaaSearchTexture,
          this.gtaoNoiseTexture
        );
      ...
      if (typeof WbWorld.instance.viewpoint === 'undefined') return;
  ...
  #parseNode(node, parentNode, isBoundingObject) {
    if (typeof this.#rootNodeId === 'undefined') this.#rootNodeId = node.id;
    ...
    switch (node.tagName) {
      ...
      case 'SpotLight':
        ...
        break;
      case 'Fog':
        if (!WbWorld.instance.hasFog) result = this.#parseFog(node, parentNode);
        else console.error('This world already has a fog.');
        break;
      case 'Transform':
      ...
      default:
        ...
        let id;
        if (typeof parentNode !== 'undefined') id = parentNode.id;
        ...
        if (typeof result !== 'undefined' && result instanceof WbGeometry) {
          if (typeof parentNode !== 'undefined') {
            if (parentNode.nodeType === WbNodeType.WB_NODE_SHAPE) {
              ...
              else parentNode.children.push(result);
            }
          }
        } else if (node.tagName === 'PBRAppearance') {
          if (
            typeof parentNode !== 'undefined' &&
            parentNode.nodeType === WbNodeType.WB_NODE_SHAPE
          ) {
          ...
          if (typeof result !== 'undefined') {
            if (
              typeof parentNode !== 'undefined' &&
              parentNode.nodeType === WbNodeType.WB_NODE_APPEARANCE
            ) {
          ...
          if (typeof result !== 'undefined') {
            if (
              typeof parentNode !== 'undefined' &&
              parentNode instanceof WbAbstractAppearance
            ) {
          ...
          console.error(
            "The parser doesn't support this type of node: " + node.tagName
          );
    }
  }
  ...
  #parseScene() {
    ...
    this.#promises.push(
      ImageLoader.loadTextureData(prefix, 'smaa_area_texture.png').then(
        image => {
          this.smaaAreaTexture = image;
          this.smaaAreaTexture.isTranslucent = false;
          this.#updatePromiseCounter(
            "Downloading assets: Texture 'smaa_area_texture.png'..."
          );
        }
      )
    );
    this.#promises.push(
      ImageLoader.loadTextureData(prefix, 'smaa_search_texture.png').then(
        image => {
          this.smaaSearchTexture = image;
          this.smaaSearchTexture.isTranslucent = false;
          this.#updatePromiseCounter(
            "Downloading assets: Texture 'smaa_search_texture.png'..."
          );
        }
      )
    );
    this.#promises.push(
      ImageLoader.loadTextureData(prefix, 'gtao_noise_texture.png').then(
        image => {
          this.gtaoNoiseTexture = image;
          this.gtaoNoiseTexture.isTranslucent = true;
          this.#updatePromiseCounter(
            "Downloading assets: Texture 'gtao_noise_texture.png'..."
          );
        }
      )
    );
  ...
  #parseWorldInfo(node) {
    ...
    WbWorld.instance.coordinateSystem = getNodeAttribute(
      node,
      'coordinateSystem',
      'ENU'
    );
    WbWorld.instance.basicTimeStep = parseInt(
      getNodeAttribute(node, 'basicTimeStep', 32)
    );
    WbWorld.instance.title = getNodeAttribute(node, 'title', 'No title');
    WbWorld.instance.description = getNodeAttribute(
      node,
      'info',
      'No description was provided for this world.'
    );
  ...
  #parseId(node) {
    ...
    if (typeof id === 'undefined') id = getAnId();
  ...
  #parseViewpoint(node, parentNode) {
    ...
    const fieldOfView = parseFloat(
      getNodeAttribute(node, 'fieldOfView', M_PI_4)
    );
    const orientation = convertStringToQuaternion(
      getNodeAttribute(node, 'orientation', '0 0 1 0')
    );
    const position = convertStringToVec3(
      getNodeAttribute(node, 'position', '0 0 10')
    );
    const exposure = parseFloat(getNodeAttribute(node, 'exposure', '1.0'));
    const bloomThreshold = parseFloat(
      getNodeAttribute(node, 'bloomThreshold', 21)
    );
    const far = parseFloat(getNodeAttribute(node, 'zFar', '2000'));
    const near = parseFloat(getNodeAttribute(node, 'zNear', '0.1'));
    const followSmoothness = parseFloat(
      getNodeAttribute(node, 'followSmoothness')
    );
    const followedId = getNodeAttribute(node, 'followedId');
    const ambientOcclusionRadius = parseFloat(
      getNodeAttribute(node, 'ambientOcclusionRadius', 2)
    );

    const viewpoint = new WbViewpoint(
      id,
      fieldOfView,
      orientation,
      position,
    ...
    if (typeof parentNode !== 'undefined') {
      if (parentNode instanceof WbGroup) parentNode.children.push(viewpoint);
  ...
  #parseBackground(node, parentNode) {
    this.#updateParserProgress(node);
    const id = this.#parseId(node);
    const skyColor = convertStringToVec3(
      getNodeAttribute(node, 'skyColor', '0 0 0')
    );
    const luminosity = parseFloat(getNodeAttribute(node, 'luminosity', '1'));

    const backgroundIdx =
      WbWorld.instance.coordinateSystem === 'ENU'
        ? [0, 1, 2, 3, 4, 5]
        : [5, 0, 1, 2, 3, 4];
    ...
    for (let i = 0; i < 6; i++) {
      ...
      // filter removes empty elements.
        ...
        backgroundUrl[i] = backgroundUrl[i].split('"').filter(element => {
    ...
    if (areUrlsPresent) {
      for (let i = 0; i < 6; i++) {
        this.#promises.push(
          ImageLoader.loadTextureData(
            this.prefix,
            backgroundUrl[backgroundIdx[i]],
            false,
            rotationValues[i]
          ).then(image => {
            this.cubeImages[cubeImageIdx[i]] = image;
            this.#updatePromiseCounter(
              "Downloading assets: Texture 'background " + i + "'..."
            );
          })
        );
      }
    ...
    for (let i = 0; i < 6; i++) {
      if (typeof backgroundIrradianceUrl[i] === 'undefined') {
        ...
        backgroundIrradianceUrl[i] = backgroundIrradianceUrl[i]
          .split('"')
          .filter(element => {
            if (element !== ' ') return element;
          })[0];
    ...
    if (areIrradianceUrlsPresent) {
      for (let i = 0; i < 6; i++) {
        this.#promises.push(
          ImageLoader.loadTextureData(
            this.prefix,
            backgroundIrradianceUrl[backgroundIdx[i]],
            true,
            rotationValues[i]
          ).then(image => {
            this.irradianceCubeURL[cubeImageIdx[i]] = image;
            this.#updatePromiseCounter(
              "Downloading assets: Texture 'background irradiance " + i + "'..."
            );
          })
        );
      }
    ...
    if (typeof parentNode !== 'undefined') {
      if (parentNode instanceof WbGroup) parentNode.children.push(background);
  ...
  #checkUse(node, parentNode) {
    ...
    if (typeof use === 'undefined') return;
    ...
    if (typeof parentNode !== 'undefined') {
      ...
      const isBoundingObject =
        getNodeAttribute(node, 'role', undefined) === 'boundingObject';
      if (
        isBoundingObject &&
        (result.nodeType === WbNodeType.WB_NODE_SHAPE ||
          result instanceof WbGroup ||
  ...
  #parsePose(node, parentNode, isBoundingObject) {
    ...
    if (typeof use !== 'undefined') return use;

    const id = this.#parseId(node);

    const translation = convertStringToVec3(
      getNodeAttribute(node, 'translation', '0 0 0')
    );
    const rotation = convertStringToQuaternion(
      getNodeAttribute(node, 'rotation', '0 0 1 0')
    );
    let name = getNodeAttribute(node, 'name', '');

    let newNode;
    if (node.tagName === 'Track') {
      const geometriesCount = parseInt(
        getNodeAttribute(node, 'geometriesCount', '10')
      );
      ...
      const numberOfRotations = parseInt(
        getNodeAttribute(node, 'numberOfRotations', '4')
      );
      newNode = new WbConnector(
        id,
        translation,
        rotation,
        name === '' ? 'connector' : name,
      ...
      const numberOfRays = parseInt(
        getNodeAttribute(node, 'numberOfRays', '1')
      );
      const lookupTableArray = convertStringToFloatArray(
        getNodeAttribute(node, 'lookupTable', '0 0 0, 0.1 1000 0')
      );
      const lookupTable = [];
      if (lookupTableArray.length % 3 === 0) {
        for (let i = 0; i < lookupTableArray.length; i = i + 3)
          lookupTable.push(
            new WbVector3(
              lookupTableArray[i],
              lookupTableArray[i + 1],
              lookupTableArray[i + 2]
            )
          );
      }
      newNode = new WbDistanceSensor(
        id,
        translation,
        rotation,
      ...
      newNode = new WbInertialUnit(
        id,
        translation,
        rotation,
        name === '' ? 'inertial unit' : name
      );
      ...
      if (tempColor.length % 3 === 0) {
        for (let i = 0; i < tempColor.length; i += 3)
          color.push(
            new WbVector3(tempColor[i], tempColor[i + 1], tempColor[i + 2])
          );
      } else console.error('Wrong number of colors in LED');
      ...
      const fieldOfView = parseFloat(
        getNodeAttribute(node, 'fieldOfView', Math.PI / 2)
      );
      const horizontalResolution = parseInt(
        getNodeAttribute(node, 'horizontalResolution', '512')
      );
      const maxRange = parseFloat(getNodeAttribute(node, 'maxRange', '1'));
      const minRange = parseFloat(getNodeAttribute(node, 'minRange', '0.01'));
      const numberOfLayers = parseInt(
        getNodeAttribute(node, 'numberOfLayers', '4')
      );
      const tiltAngle = parseFloat(getNodeAttribute(node, 'tiltAngle', '0'));
      const verticalFieldOfView = parseFloat(
        getNodeAttribute(node, 'verticalFieldOfView', '0.2')
      );
      ...
      const write =
        getNodeAttribute(node, 'write', 'true').toLowerCase() === 'true';
      newNode = new WbPen(
        id,
        translation,
        rotation,
      ...
      const horizontalFieldOfView = parseFloat(
        getNodeAttribute(node, 'horizontalFieldOfView', '0.78')
      );
      const verticalFieldOfView = parseFloat(
        getNodeAttribute(node, 'verticalFieldOfView', '0.1')
      );
      ...
      const maxRange = parseFloat(getNodeAttribute(node, 'maxRange', '1'));
      const minRange = parseFloat(getNodeAttribute(node, 'minRange', '0.01'));
      newNode = new WbRangeFinder(
        id,
        translation,
        rotation,
        name === '' ? 'range finder' : name,
        height,
        width,
        fieldOfView,
        maxRange,
      ...
      newNode = new WbTouchSensor(
        id,
        translation,
        rotation,
        name === '' ? 'touch sensor' : name
      );
    else if (node.tagName === 'VacuumGripper')
      newNode = new WbVacuumGripper(
        id,
        translation,
        rotation,
        name === '' ? 'vacuum gripper' : name
      );
    else {
      if (!isBoundingObject)
        isBoundingObject =
    ...
    if (typeof parentNode !== 'undefined') {
      ...
      if (getNodeAttribute(node, 'role', '') === 'animatedGeometry')
        ...
        parentNode.nodeType === WbNodeType.WB_NODE_SLOT ||
        parentNode instanceof WbJoint
      )
  ...
  #parseGroup(node, parentNode) {
    ...
    if (typeof parentNode !== 'undefined') {
      ...
      else parentNode.children.push(group);
    }
  ...
  #parsePropeller(node, parentNode) {
    ...
    else parentNode.children.push(propeller);
  ...
  #parseSlot(node, parentNode) {
    ...
    if (typeof parentNode !== 'undefined') {
      ...
      else parentNode.children.push(slot);
    }
  ...
  #parseJoint(node, parentNode) {
    ...
    let joint;
    if (node.tagName === 'HingeJoint') joint = new WbHingeJoint(id);
    else if (node.tagName === 'SliderJoint') joint = new WbSliderJoint(id);
    else if (node.tagName === 'Hinge2Joint') joint = new WbHinge2Joint(id);
    else if (node.tagName === 'BallJoint') joint = new WbBallJoint(id);
    ...
    if (typeof parentNode !== 'undefined') {
      ...
      else parentNode.children.push(joint);
    }
  ...
  #parseJointParameters(node, parentNode) {
    ...
    const anchor = convertStringToVec3(
      getNodeAttribute(node, 'anchor', '0 0 0')
    );
    ...
    if (node.tagName === 'JointParameters') {
      ...
      jointParameters = new WbJointParameters(
        id,
        position,
        axis,
        minStop,
      ...
      jointParameters = new WbHingeJointParameters(
        id,
        position,
        axis,
        anchor,
      ...
      jointParameters = new WbBallJointParameters(
        id,
        position,
        undefined,
        anchor,
  ...
  #parseLogicalDevice(node, parentNode) {
    ...
    else if (node.tagName === 'Brake') logicalDevice = new WbBrake(id, name);
    else if (
      node.tagName === 'RotationalMotor' ||
      node.tagName === 'LinearMotor'
    ) {
      const minPosition = parseFloat(
        getNodeAttribute(node, 'minPosition', '0')
      );
      const maxPosition = parseFloat(
        getNodeAttribute(node, 'maxPosition', '0')
      );
      const multiplier = parseFloat(getNodeAttribute(node, 'multiplier', '1'));
      if (node.tagName === 'RotationalMotor')
        logicalDevice = new WbRotationalMotor(
          id,
          name,
          minPosition,
          maxPosition,
        ...
        logicalDevice = new WbLinearMotor(
          id,
          name,
          minPosition,
          maxPosition,
  ...
  #parseShape(node, parentNode, isBoundingObject) {
    ...
    const castShadows =
      getNodeAttribute(node, 'castShadows', 'false').toLowerCase() === 'true';
    const isPickable =
      getNodeAttribute(node, 'isPickable', 'true').toLowerCase() === 'true';
    if (!isBoundingObject)
      isBoundingObject =
        getNodeAttribute(node, 'role', undefined) === 'boundingObject';
    ...
    for (let i = node.childNodes.length - 1; i >= 0; i--) {
      ...
      if (typeof appearance === 'undefined' && !isBoundingObject) {
        ...
        if (typeof appearance !== 'undefined') continue;
      }
      ...
      if (
        !(
          isBoundingObject &&
          (child.tagName === 'Appearance' || child.tagName === 'PBRAppearance')
        )
      ) {
        console.error(
          'Parser: error with node: ' +
            child.tagName +
    ...
    if (typeof parentNode !== 'undefined') {
      ...
      else parentNode.children.push(shape);
      shape.parent = parentNode.id;
    }

    if (typeof appearance !== 'undefined') appearance.parent = shape.id;
  ...
  #parseCadShape(node, parentNode) {
    ...
    if (typeof url !== 'undefined')
      url = url.split('"').filter(element => {
        if (element !== ' ') return element;
      })[0]; // filter removes empty elements

    const ccw = getNodeAttribute(node, 'ccw', 'true').toLowerCase() === 'true';
    const castShadows =
      getNodeAttribute(node, 'castShadows', 'true').toLowerCase() === 'true';
    const isPickable =
      getNodeAttribute(node, 'isPickable', 'true').toLowerCase() === 'true';

    const cadShape = new WbCadShape(
      id,
      url,
      ccw,
      castShadows,
    ...
    if (typeof parentNode !== 'undefined') {
      ...
      else parentNode.children.push(cadShape);
    }

    this.#promises.push(
      MeshLoader.loadMeshData(this.prefix, url).then(meshContent => {
  ...
  #parseBillboard(node, parentNode) {
    ...
    if (typeof parentNode !== 'undefined') parentNode.children.push(billboard);
  ...
  #parseDirectionalLight(node, parentNode) {
    ...
    const direction = convertStringToVec3(
      getNodeAttribute(node, 'direction', '0 0 -1')
    );
    const intensity = parseFloat(getNodeAttribute(node, 'intensity', '1'));
    const ambientIntensity = parseFloat(
      getNodeAttribute(node, 'ambientIntensity', '0')
    );
    const castShadows =
      getNodeAttribute(node, 'castShadows', 'false').toLowerCase() === 'true';

    const dirLight = new WbDirectionalLight(
      id,
      on,
      color,
      direction,
    ...
    if (typeof parentNode !== 'undefined' && typeof dirLight !== 'undefined') {
      ...
      else parentNode.children.push(dirLight);
    }
  ...
  #parsePointLight(node, parentNode) {
    ...
    const id = this.#parseId(node);
    const on = getNodeAttribute(node, 'on', 'true').toLowerCase() === 'true';
    const attenuation = convertStringToVec3(
      getNodeAttribute(node, 'attenuation', '1 0 0')
    );
    const color = convertStringToVec3(getNodeAttribute(node, 'color', '1 1 1'));
    const intensity = parseFloat(getNodeAttribute(node, 'intensity', '1'));
    const location = convertStringToVec3(
      getNodeAttribute(node, 'location', '0 0 0')
    );
    ...
    const pointLight = new WbPointLight(
      id,
      on,
      attenuation,
      color,
    ...
    if (
      ...
      else parentNode.children.push(pointLight);
    }
  ...
  #parseSpotLight(node, parentNode) {
    ...
    const color = convertStringToVec3(getNodeAttribute(node, 'color', '1 1 1'));
    const cutOffAngle = parseFloat(
      getNodeAttribute(node, 'cutOffAngle', '0.785')
    );
    const direction = convertStringToVec3(
      getNodeAttribute(node, 'direction', '0 0 -1')
    );
    ...
    const castShadows =
      getNodeAttribute(node, 'castShadows', 'false').toLowerCase() === 'true';
    const spotLight = new WbSpotLight(
      id,
      on,
      attenuation,
      beamWidth,
    ...
    if (typeof parentNode !== 'undefined') parentNode.children.push(spotLight);
  ...
  #parseFog(node, parentNode) {
    ...
    const id = this.#parseId(node);
    const color = convertStringToVec3(getNodeAttribute(node, 'color', '1 1 1'));
    const visibilityRange = parseFloat(
      getNodeAttribute(node, 'visibilityRange', '0')
    );
    ...
    if (typeof fog !== 'undefined') WbWorld.instance.hasFog = true;

    if (typeof parentNode !== 'undefined') parentNode.children.push(fog);
  ...
  #parseGeometry(node, parentId) {
    ...
    let geometry;
    if (node.tagName === 'Box') geometry = this.#parseBox(node, id);
    else if (node.tagName === 'Sphere') geometry = this.#parseSphere(node, id);
    else if (node.tagName === 'Cone') geometry = this.#parseCone(node, id);
    else if (node.tagName === 'Plane') geometry = this.#parsePlane(node, id);
    ...
    else if (node.tagName === 'Mesh') geometry = this.#parseMesh(node, id);
  ...
  #parseCone(node, id) {
    this.#updateParserProgress(node);
    const bottomRadius = parseFloat(
      getNodeAttribute(node, 'bottomRadius', '1')
    );
    const height = parseFloat(getNodeAttribute(node, 'height', '2'));
    const subdivision = parseInt(getNodeAttribute(node, 'subdivision', '32'));
    const side =
      getNodeAttribute(node, 'side', 'true').toLowerCase() === 'true';
    const bottom =
      getNodeAttribute(node, 'bottom', 'true').toLowerCase() === 'true';

    const cone = new WbCone(
      id,
      bottomRadius,
      height,
      subdivision,
  ...
  #parseIndexedFaceSet(node, id) {
    this.#updateParserProgress(node);
    const coordIndex = convertStringToFloatArray(
      getNodeAttribute(node, 'coordIndex', '')
    );
    const normalIndex = convertStringToFloatArray(
      getNodeAttribute(node, 'normalIndex', '')
    );
    const texCoordIndex = convertStringToFloatArray(
      getNodeAttribute(node, 'texCoordIndex', '')
    );
    ...
    const textureCoordinateNode = node.getElementsByTagName(
      'TextureCoordinate'
    );
    let texCoord;
    ...
    let normal;
    if (normalNode.length > 0) normal = this.#parseNormal(normalNode[0]);

    const ccw = getNodeAttribute(node, 'ccw', 'true').toLowerCase() === 'true';
    const normalPerVertex =
      getNodeAttribute(node, 'normalPerVertex', 'true').toLowerCase() ===
      'true';
    const creaseAngle = parseFloat(getNodeAttribute(node, 'creaseAngle', '0'));
    const ifs = new WbIndexedFaceSet(
      id,
      coordIndex,
      normalIndex,
      texCoordIndex,
    ...
    if (typeof coord !== 'undefined') coord.parent = ifs.id;

    if (typeof texCoord !== 'undefined') texCoord.parent = ifs.id;

    if (typeof normal !== 'undefined') normal.parent = ifs.id;
    return ifs;
  }
  ...
  #parseIndexedLineSet(node, id) {
    ...
    if (typeof coord !== 'undefined') coord.parent = ils.id;
  ...
  #parsePointSet(node, id) {
    ...
    let color;
    if (colorNode.length > 0) color = this.#parseColor(colorNode[0]);

    const ps = new WbPointSet(id, coord, color);

    if (typeof color !== 'undefined') color.parent = ps.id;

    if (typeof coord !== 'undefined') coord.parent = ps.id;
  ...
  #parseColor(node) {
    ...
    const id = this.#parseId(node);
    let colors = [];
    const colorArray = convertStringToFloatArray(
      getNodeAttribute(node, 'color', '')
    );
    if (typeof colorArray !== 'undefined') {
      for (let i = 0; i < colorArray.length; i += 3)
        colors.push(
          new WbVector3(colorArray[i], colorArray[i + 1], colorArray[i + 2])
        );
    }
  ...
  #parseCoordinate(node) {
    ...
    const id = this.#parseId(node);
    let point = [];
    const pointArray = convertStringToFloatArray(
      getNodeAttribute(node, 'point', '')
    );
    if (typeof pointArray !== 'undefined') {
      for (let i = 0; i < pointArray.length; i += 3)
        point.push(
          new WbVector3(pointArray[i], pointArray[i + 1], pointArray[i + 2])
        );
    }
    ...
    if (url) {
      this.#promises.push(
        MeshLoader.loadMeshData(this.prefix, url).then(meshContent => {
          ...
          this.#updatePromiseCounter(
            "Downloading assets: Mesh 'mesh " + name + "'..."
          );
        })
      );
      this.#promiseNumber += 1;
  ...
  #parseAppearance(node, parentId) {
    ...
    if (typeof appearance !== 'undefined') {
      if (typeof material !== 'undefined') material.parent = appearance.id;

      if (typeof texture !== 'undefined') texture.parent = appearance.id;

      if (typeof transform !== 'undefined') transform.parent = appearance.id;

      if (typeof parentId !== 'undefined') appearance.parent = parentId;
  ...
  #parseMaterial(node, parentId) {
    ...
    const ambientIntensity = parseFloat(
      getNodeAttribute(node, 'ambientIntensity', '0.2')
    );
    const diffuseColor = convertStringToVec3(
      getNodeAttribute(node, 'diffuseColor', '0.8 0.8 0.8')
    );
    const specularColor = convertStringToVec3(
      getNodeAttribute(node, 'specularColor', '0 0 0')
    );
    const emissiveColor = convertStringToVec3(
      getNodeAttribute(node, 'emissiveColor', '0 0 0')
    );
    const shininess = parseFloat(getNodeAttribute(node, 'shininess', '0.2'));
    const transparency = parseFloat(
      getNodeAttribute(node, 'transparency', '0')
    );

    const material = new WbMaterial(
      id,
      ambientIntensity,
      diffuseColor,
      specularColor,
    ...
    if (typeof parentId !== 'undefined') material.parent = parentId;
  ...
  #parseImageTexture(node, parentId) {
    ...
    const s =
      getNodeAttribute(node, 'repeatS', 'true').toLowerCase() === 'true';
    const t =
      getNodeAttribute(node, 'repeatT', 'true').toLowerCase() === 'true';
    ...
    if (typeof url !== 'undefined' && !this.#downloadingImage.has(url)) {
      ...
      this.#promises.push(
        ImageLoader.loadImageTextureInWren(imageTexture, this.prefix, url)
      );
    }

    if (typeof parentId !== 'undefined') imageTexture.parent = parentId;
  ...
  #parsePbrAppearance(node, parentId) {
    ...
    const baseColor = convertStringToVec3(
      getNodeAttribute(node, 'baseColor', '1 1 1')
    );
    const transparency = parseFloat(
      getNodeAttribute(node, 'transparency', '0')
    );
    ...
    const normalMapFactor = parseFloat(
      getNodeAttribute(node, 'normalMapFactor', '1')
    );
    const occlusionMapStrength = parseFloat(
      getNodeAttribute(node, 'occlusionMapStrength', '1')
    );
    const emissiveColor = convertStringToVec3(
      getNodeAttribute(node, 'emissiveColor', '0 0 0')
    );
    const emissiveIntensity = parseFloat(
      getNodeAttribute(node, 'emissiveIntensity', '1')
    );
    ...
    for (let i = 0; i < imageTextures.length; i++) {
      ...
      if (role === 'baseColorMap') {
        ...
        if (typeof normalMap !== 'undefined') normalMap.role = 'normalMap';
      } else if (role === 'occlusionMap') {
    ...
    const pbrAppearance = new WbPbrAppearance(
      id,
      baseColor,
      baseColorMap,
      transparency,
    ...
    if (typeof pbrAppearance !== 'undefined') {
      if (typeof transform !== 'undefined') transform.parent = pbrAppearance.id;
      ...
      if (typeof normalMap !== 'undefined') normalMap.parent = pbrAppearance.id;
      ...
      if (typeof parentId !== 'undefined') pbrAppearance.parent = parentId;
  ...
  #parseTextureTransform(node, parentId) {
    ...
    const translation = convertStringToVec2(
      getNodeAttribute(node, 'translation', '0 0')
    );

    const textureTransform = new WbTextureTransform(
      id,
      center,
      rotation,
      scale,
    ...
    if (typeof parentId !== 'undefined') textureTransform.parent = parentId;
...
function convertStringToFloatArray(string) {
  ...
  if (stringList) return stringList.map(element => parseFloat(element));
}
