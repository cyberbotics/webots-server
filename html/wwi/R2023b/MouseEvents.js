...
export default class MouseEvents {
  ...
  constructor(scene, domElement, mobileDevice) {
    ...
    this.#domElement.parentNode.addEventListener(
      'contextmenu',
      event => {
        event.preventDefault();
      },
  ...
  init() {
    if (typeof this.picker === 'undefined') this.picker = new WbWrenPicker();
  }
  ...
  #onMouseDown(event) {
    ...
    const position = MouseEvents.convertMouseEventPositionToRelativePosition(
      canvas,
      this.#state.x,
      this.#state.y
    );
    ...
    if (this.picker.selectedId !== -1) {
      this.#rotationCenter = new WbVector3(
        (this.picker.coordinates.x / canvas.width) * 2 - 1,
        (this.picker.coordinates.y / canvas.height) * 2 - 1,
        this.picker.coordinates.z
      );
      this.#rotationCenter = WbWorld.instance.viewpoint.toWorld(
        this.#rotationCenter
      );
      this.#rotationCenter = glm.vec3(
      ...
      this.#rotationCenter = glm.vec3(
        WbWorld.instance.viewpoint.position.x,
        WbWorld.instance.viewpoint.position.y,
  ...
  #onMouseMove(event) {
    ...
    if (typeof this.#state.x === 'undefined')
      ...
      return;
    if ('buttons' in event) this.#state.mouseDown = event.buttons;
    ...
    if (this.#state.mouseDown === 1) {
      ...
      const pitchRotation = glm.quat(
        Math.cos(halfPitchAngle),
        sinusPitch * pitch.x,
        sinusPitch * pitch.y,
        sinusPitch * pitch.z
      );
      const worldUpVector = WbWorld.instance.upVector;
      const yawRotation = glm.quat(
        Math.cos(halfYawAngle),
        sinusYaw * worldUpVector.x,
        sinusYaw * worldUpVector.y,
        sinusYaw * worldUpVector.z
      );
      // Updates camera's position and orientation
      const deltaRotation = yawRotation.mul(pitchRotation);
      const currentPosition = deltaRotation
        .mul(
          glm.vec3(position.x, position.y, position.z).sub(this.#rotationCenter)
        )
        .add(this.#rotationCenter);
      const currentOrientation = deltaRotation.mul(
        vec4ToQuaternion(orientation)
      );
      WbWorld.instance.viewpoint.position = new WbVector3(
        currentPosition.x,
        currentPosition.y,
        currentPosition.z
      );
      WbWorld.instance.viewpoint.orientation = quaternionToVec4(
        currentOrientation
      );
      ...
      else distanceToPickPosition = length(position);

      if (distanceToPickPosition < 0.001) distanceToPickPosition = 0.001;

      const scaleFactor =
        (distanceToPickPosition *
          2 *
          Math.tan(WbWorld.instance.viewpoint.fieldOfView / 2)) /
      ...
      if (this.#state.mouseDown === 2) {
        ...
        const zDisplacement = rollVector.mul(
          scaleFactor * -5 * this.#moveParams.dy
        );
        const roll2 = fromAxisAngle(
          rollVector.x,
          rollVector.y,
          rollVector.z,
          0.01 * this.#moveParams.dx
        );
        ...
        WbWorld.instance.viewpoint.orientation = quaternionToVec4(
          roll3.mul(vec4ToQuaternion(orientation))
        );
        WbWorld.instance.viewpoint.updatePosition();
        WbWorld.instance.viewpoint.updateOrientation();
    ...
    this.#state.moved =
      event.clientX !== this.#state.x || event.clientY !== this.#state.y;
  ...
  #onMouseWheel(event) {
    ...
    if (!this.#enableNavigation || this.#state.wheelFocus === false) {
      ...
      if (this.#state.wheelTimeout) {
        ...
        this.#state.wheelTimeout = setTimeout(event => {
          this.#wheelTimeoutCallback(event);
        }, 1500);
      }
      return;
    }
    ...
    const scaleFactor =
      0.1 * (event.deltaY < 0.0 ? 1 : -1) * distanceToPickPosition;
  ...
  #onMouseOver(event) {
    ...
    if (typeof this.showToolbar !== 'undefined') this.showToolbar();
  }
  ...
  onMouseLeave(event) {
    ...
    if (typeof this.hideToolbar !== 'undefined') this.hideToolbar();
  }
  ...
  #onTouchStart(event) {
    ...
    if (event.targetTouches.length === 2) {
      ...
      this.#state.touchOrientation = Math.atan2(
        this.#state.y1 - this.#state.y,
        this.#state.x1 - this.#state.x
      );
  ...
  #clearMouseMove() {
    ...
    this.#state.longClick =
      Date.now() - this.#state.initialTimeStamp >= timeDelay;
  ...
  #selectAndHandleClick() {
    if (
      this.#state.moved === false &&
      (!this.#state.longClick || this.#mobileDevice)
    ) {
      Selector.select(this.picker.selectedId);

      if (
        typeof WbWorld.instance.nodes.get(Selector.selectedId) !== 'undefined'
      )
        WbWorld.instance.nodes
          .get(Selector.selectedId)
          .updateBoundingObjectVisibility();

      if (
        typeof WbWorld.instance.nodes.get(Selector.previousId) !== 'undefined'
      )
        WbWorld.instance.nodes
          .get(Selector.previousId)
          .updateBoundingObjectVisibility();
...
MouseEvents.convertMouseEventPositionToRelativePosition = (
  element,
  eventX,
  eventY
) => {
