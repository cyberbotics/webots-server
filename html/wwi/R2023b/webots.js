...
webots.View = class View {
  ...
  constructor(view3D, mobile) {
    ...
    this.onresize = () => {
      if (typeof this.x3dScene !== 'undefined') this.x3dScene.resize();
      ...
      for (let i = labels.length - 1; i >= 0; i--) {
        ...
        element.style.fontSize =
          (this.#getHeight(this.#x3dDiv) * element.size) / 2.25 + 'px';
  ...
  open(url, mode, thumbnail, raw) {
    ...
    const initWorld = async () => {
      ...
      if (this.#isWebSocketProtocol) {
        if (this.url.endsWith('.wbt')) {
          ...
          // url expected form: "ws://cyberbotics1.epfl.ch:80"
        ...
        // assuming it's an URL to a .x3d file
        if (raw)
          await this.x3dScene.loadRawWorldFile(
            this.url,
            finalizeWorld,
            this.progress
          );
        else
          await this.x3dScene.loadWorldFile(
            this.url,
            finalizeWorld,
            this.progress
          );
    ...
    this.#isWebSocketProtocol =
      ...
      this.url.endsWith('.wbt');

    let texturePathPrefix;
    if (!raw)
      texturePathPrefix = url.includes('/')
    ...
    if (
      typeof this.x3dScene !== 'undefined' &&
      typeof this.mouseEvents === 'undefined'
    ) {
  ...
  close() {
    if (this.multimediaClient) this.multimediaClient.disconnect();
    if (this.#server && this.#server.socket) this.#server.socket.close();
  ...
  updateWorldList(currentWorld, worlds) {
    ...
    if (existingCurrentWorld) {
      ...
      if (
        webotsView &&
        typeof webotsView.toolbar !== 'undefined' &&
  ...
  setLabel(properties) {
    ...
    labelElement.style.fontSize =
      (this.#getHeight(this.#x3dDiv) * properties.size) / 2.25 + 'px';
    labelElement.style.left =
      this.#getWidth(this.#x3dDiv) * properties.x + 'px';
    labelElement.style.top =
      this.#getHeight(this.#x3dDiv) * properties.y + 'px';
    ...
    if (properties.text && properties.text.includes('█')) {
      properties.text = properties.text.replaceAll(
        '█',
        '<span style="background:' + labelElement.style.color + '"> </span>'
      );
  ...
  destroyWorld() {
    ...
    if (
      typeof this.mouseEvents !== 'undefined' &&
      typeof this.mouseEvents.picker !== 'undefined'
    ) {
